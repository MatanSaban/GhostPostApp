...
/**
 * GET /api/dashboard/stats/ga-kpis?siteId=xxx&startDate=YYYY-MM-DD&endDate=YYYY-MM-DD
 * Returns GA4 KPI data (visitors, pageViews, sessions, avgSessionDuration) for a custom date range
 */
export async function GET(request) {
  try {
    ...
    if (!integration || !integration.gaConnected || !integration.gaPropertyId) {
      return NextResponse.json({ ga: null });
    }
    ...
    if (needsRefresh) {
      try {
        ...
        console.error('[GA KPIs] Token refresh failed:', err);
        return NextResponse.json({ ga: null, tokenError: true });
      }
    }
    ...
    console.error('[GA KPIs] Error:', error);
