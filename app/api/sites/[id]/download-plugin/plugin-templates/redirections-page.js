...
$external_redirects = ($primary_plugin && $primary_plugin !== 'ghostseo') ? $manager->get_all_redirects() : array();
...
<div class="wrap gp-wrap gp-redirections-page <?php echo esc_attr($theme_class); ?>" dir="<?php echo esc_attr($dir); ?>">
    ...
    <div class="gp-header">
        ...
        <h1 class="gp-header-title"><?php esc_html_e('Redirections', 'ghostseo-connector'); ?></h1>
    </div>
    ...
    <div class="gp-recommendation-banner">
        ...
        <div class="gp-recommendation-content">
            ...
            <p>
                ...
                printf(
                    esc_html__('We detected %s on your site. We recommend importing your existing redirects into GhostSEO and then deactivating the external plugin to avoid conflicts and improve performance.', 'ghostseo-connector'),
            ...
            <div class="gp-recommendation-steps">
                <div class="gp-step">
                    ...
                    <span><?php esc_html_e('Import existing redirects', 'ghostseo-connector'); ?></span>
                </div>
                <div class="gp-step">
                    <span class="gp-step-number">2</span>
                    <span><?php esc_html_e('Verify redirects are working', 'ghostseo-connector'); ?></span>
                </div>
                <div class="gp-step">
                    <span class="gp-step-number">3</span>
                    <span><?php esc_html_e('Deactivate the external plugin', 'ghostseo-connector'); ?></span>
                </div>
            </div>
            <div class="gp-recommendation-actions">
                <button type="button" id="gp-import-redirects" class="gp-btn gp-btn-primary">
                    ...
                    <?php printf(
                        esc_html__('Import %d Redirects', 'ghostseo-connector'),
                ...
                <button type="button" class="gp-btn gp-btn-secondary gp-deactivate-plugin" data-slug="<?php echo esc_attr($dp['file']); ?>" data-name="<?php echo esc_attr($dp['name']); ?>">
                    ...
                    <?php printf(esc_html__('Deactivate %s', 'ghostseo-connector'), esc_html($dp['name'])); ?>
                </button>
                <?php endif; ?>
                <?php endforeach; ?>
                <span class="gp-import-count">
                    <?php printf(
                        esc_html__('%d redirects found in %s', 'ghostseo-connector'),
                        count($external_redirects),
    ...
    <div class="gp-card gp-create-redirect-card">
        <h2><?php esc_html_e('Add New Redirect', 'ghostseo-connector'); ?></h2>
            ...
            <table class="gp-redirects-table widefat">
                <thead>
                    <tr>
                        ...
                        <th class="gp-col-source"><?php esc_html_e('From', 'ghostseo-connector'); ?></th>
                        <th class="gp-col-target"><?php esc_html_e('To', 'ghostseo-connector'); ?></th>
                    ...
                    <tr data-id="<?php echo esc_attr($redirect['id']); ?>" class="<?php echo empty($redirect['is_active']) ? 'gp-inactive-row' : ''; ?>">
                        <td class="gp-col-status">
                            <button type="button" class="gp-toggle-status <?php echo $redirect['is_active'] ? 'gp-active' : 'gp-not-active'; ?>" data-id="<?php echo esc_attr($redirect['id']); ?>" data-active="<?php echo $redirect['is_active'] ? '1' : '0'; ?>">
                                ...
                                <span class="gp-status-label"><?php echo $redirect['is_active'] ? esc_html__('Active', 'ghostseo-connector') : esc_html__('Inactive', 'ghostseo-connector'); ?></span>
                            </button>
                        </td>
