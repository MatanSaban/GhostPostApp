 ...
 * 6. Each plan includes Ai-GCoins, seats, and websites. Add-ons can extend these.
...
/**
 * Check if a feature exists in the plan's features array
 * Features array is: [{ key: string, label: string }, ...]
 * If the key exists in the array, the feature is enabled
 * @param {Array} features - The features array from plan
 * @param {string} key - The key to look for (e.g., 'whiteLabelReports')
 * @returns {boolean} - True if feature exists in the array
 */
 ...
 * @param {object} plan - The plan object with limitations and features arrays
 * @returns {object} - Object with all limit values and feature flags
 */
export function getPlanLimits(plan) {
  ...
  return {
    ...
    // Boolean features (from features array)
    // Whether this plan includes branded/white-label client reports.
    // Accept any of the key spellings seen in the wild (canonical camelCase,
    // snake_case from older seeds, shorter `white_label` etc.) AND fall back
    // to the plan slug — by convention "enterprise" always includes this
    // feature, regardless of whether the features JSON happens to name it.
    // Without that fallback, Enterprise accounts whose plan row was
    // reseeded/edited without the feature key were told to upgrade.
    whiteLabelReports:
      hasFeature(features, 'whiteLabelReports') ||
      hasFeature(features, 'white_label') ||
      hasFeature(features, 'white_label_reports') ||
      hasFeature(features, 'whiteLabel') ||
  ...
  for (const purchase of addOnPurchases) {
    ...
    switch (addOn.type) {
      ...
      case 'SITES':
        ...
        break;
      case 'AI_CREDITS':
        // One-time Ai-GCoins are tracked in creditsRemaining
        if (addOn.billingType === 'ONE_TIME') {
          additionalAiCredits += purchase.creditsRemaining || 0;
        } else {
          // Recurring Ai-GCoins add to monthly allocation
...
export async function canPurchaseAddOn(accountId, addOnType, quantity = 1) {
  ...
  if (!resources.hasSubscription) {
    return { 
      ...
      reasonKey: 'settings.addonsSection.errors.noSubscription',
    };
  }
  
  // Ai-GCoins are unlimited
    ...
    if (maxAddOnSeats !== null) {
      ...
      if (currentAddOnSeats + quantity > maxAddOnSeats) {
        return {
          ...
          reasonKey: 'settings.addonsSection.errors.maxSeatsReached',
          reasonParams: { max: maxAddOnSeats, current: currentAddOnSeats },
        };
      }
    }
  }
    ...
    if (maxAddOnSites !== null) {
      ...
      if (currentAddOnSites + quantity > maxAddOnSites) {
        return {
          ...
          reasonKey: 'settings.addonsSection.errors.maxSitesReached',
          reasonParams: { max: maxAddOnSites, current: currentAddOnSites },
        };
      }
    }
  }
...
/**
 * Deduct Ai-GCoins from an account
 * @param {string} accountId - The account ID
...
export async function deductAiCredits(accountId, amount, options = {}) {
  const { userId, siteId, source = 'usage', description, metadata } = options;

  try {
    // 1. Soft availability check (no transaction - just a read)
    const account = await prisma.account.findUnique({
      where: { id: accountId },
      select: {
    ...
    let available = account.aiCreditsBalance;

    if (available <= 0) {
      const planLimitations = account.subscription?.plan?.limitations || [];
      const aiCreditsLimit = getLimitFromPlan(planLimitations, 'aiCredits', 0);

      if (aiCreditsLimit === null) {
        available = Infinity;
      } else if (aiCreditsLimit > 0) {
        available = Math.max(0, aiCreditsLimit - (account.aiCreditsUsedTotal || 0));
      }
    }

    if (available < amount) {
      console.warn('[deductAiCredits] Insufficient credits:', {
        accountId, available, requested: amount,
      });
      return { success: false, error: 'Insufficient Ai-GCoins' };
    }

    // 2. Atomic update with retry - $inc is document-level atomic but
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      try {
        updated = await prisma.account.update({
          ...
          select: { aiCreditsBalance: true, aiCreditsUsedTotal: true },
        });
        break; // success
      } catch (updateError) {
    ...
    // 3. Append log entry (separate collection - no conflict with step 2)
    await prisma.aiCreditsLog.create({
      data: {
        ...
        type: 'DEBIT',
        amount,
        balance: updated.aiCreditsBalance,
        source,
        description,
        ...(metadata ? { metadata } : {}),
      },
    ...
    return { success: true, balance: updated.aiCreditsBalance, usedTotal: updated.aiCreditsUsedTotal };
  } catch (error) {
    console.error('[deductAiCredits] Error:', error.message, '| accountId:', accountId, '| amount:', amount);
    return { success: false, error: error.message };
  }
}

/**
 * Add Ai-GCoins to an account
 * @param {string} accountId - The account ID
...
/**
 * Get all Ai-GCoins packs available for purchase
 * @returns {Promise<Array>} - Available Ai-GCoins add-ons
 */
