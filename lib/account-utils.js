...
export async function deductAiCredits(accountId, amount, options = {}) {
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      // Get current account with subscription plan to compute real available credits
      ...
      // Primary: use aiCreditsBalance if it was explicitly loaded (> 0)
      // Fallback: derive from plan limit minus total used
      let available = account.aiCreditsBalance;
      
      if (available <= 0) {
        // aiCreditsBalance was never seeded — compute from plan allocation
      ...
      if (available < amount) {
        console.warn('[deductAiCredits] Insufficient credits:', {
          ...
          usedTotal: account.aiCreditsUsedTotal,
          planLimit: getLimitFromPlan(account.subscription?.plan?.limitations || [], 'aiCredits', 0),
          available,
          requested: amount,
        });
      ...
      const newBalance = available === Infinity ? 0 : Math.max(0, account.aiCreditsBalance - amount);
      
      // Update balance and usage counter
      ...
      const newUsedTotal = (account.aiCreditsUsedTotal || 0) + amount;
      return { success: true, balance: newBalance, usedTotal: newUsedTotal };
    });
    
    return result;
  } catch (error) {
    if (!error.message?.includes('Insufficient')) {
      console.error('[deductAiCredits] Unexpected error:', error.message, '| accountId:', accountId, '| amount:', amount);
    }
