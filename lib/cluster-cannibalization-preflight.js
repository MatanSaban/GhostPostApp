 ...
 * @returns {Promise<{ results: Array<{ hasConflict: boolean, conflicts: Array<{entityId, entityTitle, entityUrl, score, type, recommendation}> }> }>}
 */
export async function preflightCandidates({
  ...
  if (!topicClusterId) {
    return emptyResults(candidates);
  }
  ...
  if (!cluster || !cluster.memberEntityIds?.length) {
    return emptyResults(candidates);
  }
 ...
 */
export async function preflightCandidate({ candidate, topicClusterId, accountId, userId, siteId }) {
