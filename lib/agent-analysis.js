...
function buildDedupKey(titleKey, data) {
  ...
  // Per-cluster cannibalization: use sorted URL paths (not full URLs) so partial fixes
  // that remove some URLs still match the same cluster when remaining URLs overlap
  if (titleKey.includes('cannibalization')) {
    const urls = d.issues?.[0]?.urls || [];
    const sortedUrls = [...urls].sort().join('|');
    return `${titleKey}:${sortedUrls}`;
  }
  // Per-engine AI-coverage insights
  if (titleKey.includes('aiCitedByEngine')) return `${titleKey}:${d.engine || ''}`;
  // Per-page AI-coverage insights (gap, missing schema, not concise)
 ...
 * @param {string} [userId] - Optional user ID for AI credit tracking (null for cron jobs)
 * @returns {{ runId: string, insightsCount: number, resolvedCount: number }}
 */
export async function runSiteAnalysis(siteId, accountId, source = 'cron', existingRunId = null, userId = null) {
  let run;
  if (existingRunId) {
  ...
  try {
    ...
    for (const e of existingInsights) {
      ...
      if (currentDedupKeys.has(key)) {
        // Exact match — insight still detected with same URL set
...
async function analyzePostImages(site, batchId) {
  ...
  // Fetch published entities with content and featuredImage
  const rawEntities = await prisma.siteEntity.findMany({
    where: { siteId: site.id, status: 'PUBLISHED' },
    select: {
...
async function fetchPageHtmlSafe(url) {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': 'Mozilla/5.0 (compatible; GhostPostAgent/1.0)' },
      redirect: 'follow',
    });
    clearTimeout(timer);
...
async function analyzeH1Tags(site, batchId) {
  ...
  for (const entity of entities) {
    ...
    const h1Matches = entity.content.match(/<h1[\s>]/gi) || [];
