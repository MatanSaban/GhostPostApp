    ...
    for (let i = 0; i < supportingPages.length; i++) {
      ...
      try {
        const diff = await generatePageDifferentiation(
          sp, alphaPage, blacklist, siteId, allPageIds, siteLanguage || 'en', job.accountId
        );
...
export async function executeDifferentiationFixes({ jobId, userId, accountId, siteId }) {
  ...
  // Deduct AI credits
  const { deductAiCredits } = await import('../account-utils.js');
  await deductAiCredits(accountId, totalCredits, {
    source: 'content_differentiation',
    sourceId: jobId,
    userId,
    siteId,
    description: `Content differentiation: ${successfulPages.length} pages`,
  });
