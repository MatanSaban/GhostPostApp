...
import { getFeatureCreditCost } from './ai-pricing.js';

// ═══ CONSTANTS ═══════════════════════════════════════════════════════════════

const MAX_AI_RETRIES = 2;
const OVERLAP_THRESHOLD = 0.5; // 50% word overlap = too similar
const DEFAULT_CREDITS_PER_PAGE = 25; // Fallback if DB pricing unavailable
...
async function processDifferentiationJob(jobId) {
  try {
    ...
    const creditsPerPage = await getFeatureCreditCost('CANNIBALIZATION_FIX').catch(() => DEFAULT_CREDITS_PER_PAGE);
    const totalCredits = successfulDiffs.length * creditsPerPage;
 ...
 * Deducts Ai-GCoins, updates local SiteEntity records,
...
export async function executeDifferentiationFixes({ jobId, userId, accountId, siteId }) {
  ...
  const creditsPerPageExec = await getFeatureCreditCost('CANNIBALIZATION_FIX').catch(() => DEFAULT_CREDITS_PER_PAGE);
  const totalCredits = summary?.estimatedCredits || successfulPages.length * creditsPerPageExec;
