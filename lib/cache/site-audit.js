...
import { getAllPageResults } from '@/lib/audit/page-results-helper.js';
  ...
  if (!isTerminal) {
    ...
    return prisma.siteAudit.findFirst({
      ...
      select: {
        ...
        chunkErrors: true, siteWideIndicators: true,
        startedAt: true, completedAt: true, createdAt: true, updatedAt: true,
      },
    });
  }

  // Phase 1+2: pageResults AND issues moved out of the audit doc into their
  // own collections (AuditPageResult, AuditIssue). The cached fetch returns
  // the audit shell — we inline pageResults from the helper after the cache
  // hit. Phase 4b: issues are NOT inlined here — they're fetched on demand
  // by the dashboard via /api/audit/issues/aggregate (overview totals) +
  // /api/audit/issues (drill-down / per-tab / per-page lazy fetches). This
  // shrinks the audit response from 1-2MB to ~10KB for sites with many
  // issues, drops Cache-Control egress, and makes payload size independent
  // of audit issue count.
  const cached = unstable_cache(
    async () => {
      // Skip embedded issues + pageResults — pageResults helper sources them;
      // issues are fetched via dedicated endpoints by the client.
      return prisma.siteAudit.findFirst({
        where: { id: auditId, siteId },
        select: {
          id: true, siteId: true, status: true, score: true, deviceType: true,
          categoryScores: true, pagesScanned: true, pagesFound: true,
          discoveryMethod: true, screenshots: true, summary: true,
          summaryTranslations: true, issueTranslations: true, fixPreviews: true,
          progress: true, phase: true, chunkErrors: true, siteWideIndicators: true,
          startedAt: true, completedAt: true, createdAt: true, updatedAt: true,
        },
      });
    },
