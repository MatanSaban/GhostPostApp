...
// Integration scopes. `drive.readonly` is used so the server can list the
// user's Drive files for our custom platform-themed file browser - the
// narrower `drive.file` scope only exposes files the user picks through the
// Google-hosted Picker widget, which we don't use.
...
function getBaseUrl() {
  ...
  return process.env.BASE_URL || process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3001';
}
 ...
 * Uses Webmasters API: PUT /webmasters/v3/sites/{siteUrl}/sitemaps/{feedpath}
 * Requires `webmasters` (read+write) scope.
 * @param {string} accessToken
 * @param {string} siteUrl - The GSC site URL (e.g., "https://example.com/")
 * @param {string} sitemapUrl - Full sitemap URL to submit
 * @returns {{ success: boolean, error?: string }}
 */
export async function submitGSCSitemap(accessToken, siteUrl, sitemapUrl) {
  try {
    const encodedSiteUrl = encodeURIComponent(siteUrl);
    ...
    if (res.ok || res.status === 204) {
      console.log(`[Google Integration] Sitemap submitted: ${sitemapUrl}`);
      return { success: true };
    }

    const errorBody = await res.text().catch(() => '');
    console.error(`[Google Integration] Sitemap submission failed for ${sitemapUrl}: ${res.status}`, errorBody);

    // 403 usually means scope is readonly - user needs to reconnect
    if (res.status === 403) {
      return { success: false, error: 'SCOPE_INSUFFICIENT' };
    }
    return { success: false, error: `HTTP ${res.status}` };
  } catch (e) {
    console.error('[Google Integration] Error submitting sitemap:', e.message);
    return { success: false, error: e.message };
  }
}

/**
 * Verify a sitemap is actually submitted to GSC by checking the sitemaps list.
 * @param {string} accessToken
 * @param {string} siteUrl
 * @param {string} sitemapUrl
 * @returns {boolean}
 */
export async function verifySitemapSubmitted(accessToken, siteUrl, sitemapUrl) {
  const sitemaps = await listGSCSitemaps(accessToken, siteUrl);
  return sitemaps.some(s => s === sitemapUrl || s.replace(/\/$/, '') === sitemapUrl.replace(/\/$/, ''));
}

/**
 * Best-effort auto-selection of the GA property + GSC site after the
 * interview OAuth flow. Matches by hostname and updates the GoogleIntegration
 * row with the same field shapes as the save-ga / save-gsc settings actions.
 * Never throws.
 * @returns {Promise<{gaSelected: boolean, gscSelected: boolean}>}
 */
export async function autoSelectGoogleServices({ siteId, siteUrl, accessToken, scopes = [] }) {
  const result = { gaSelected: false, gscSelected: false };
  if (!siteId || !siteUrl || !accessToken) return result;
  ...
  // Bare hostname of a GSC entry ("sc-domain:example.com" or "https://example.com/")
  const gscEntryHost = (entryUrl) => {
    if (entryUrl.startsWith('sc-domain:')) {
      return entryUrl.slice('sc-domain:'.length).replace(/^www\./, '').toLowerCase();
    }
    try {
      return new URL(entryUrl).hostname.replace(/^www\./, '').toLowerCase();
    } catch {
      return null;
    }
  };

  // ── GSC: pick the site that matches the hostname ──
  if (scopes.some((s) => s.includes('webmasters'))) {
    try {
      const sites = await listGSCSites(accessToken);
      const domainMatch = sites.find((s) => s.siteUrl === `sc-domain:${host}`);
      const prefixMatches = sites.filter(
        (s) => !s.siteUrl.startsWith('sc-domain:') && gscEntryHost(s.siteUrl) === host
      );
      const prefixMatch =
        prefixMatches.find((s) => s.siteUrl.startsWith('https://')) || prefixMatches[0];
      const subdomainMatch = sites.find((s) => {
        const h = gscEntryHost(s.siteUrl);
        return h && h.endsWith(`.${host}`);
      });
      const match = domainMatch || prefixMatch || subdomainMatch;
      if (match) {
        await prisma.googleIntegration.update({
          where: { siteId },
          data: { gscConnected: true, gscSiteUrl: match.siteUrl },
        });
        result.gscSelected = true;
        console.log('[Google Integration] Auto-selected GSC site:', match.siteUrl);
      }
    } catch (err) {
      console.warn('[Google Integration] GSC auto-select failed:', err.message);
    }
  }

  // ── GA: pick the property whose display name matches the hostname ──
  if (scopes.some((s) => s.includes('analytics'))) {
    try {
      const properties = await listGAProperties(accessToken);
      // Registrable label, e.g. "example" for blog.example.co.il
      const parts = host.split('.');
      const sharedSlds = ['co', 'com', 'org', 'net', 'gov', 'ac', 'or'];
      let label = parts.length >= 2 ? parts[parts.length - 2] : parts[0];
      if (sharedSlds.includes(label) && parts.length >= 3) {
        label = parts[parts.length - 3];
      }
      let match = properties.find((p) => {
        const name = (p.name || '').toLowerCase();
        return name.includes(host) || (label.length > 2 && name.includes(label));
      });
      // Single property → safe to assume it's this site
      if (!match && properties.length === 1) match = properties[0];
      if (match) {
        await prisma.googleIntegration.update({
          where: { siteId },
          data: {
            gaConnected: true,
            gaPropertyId: match.id,
            gaPropertyName: match.name || match.id,
          },
        });
        result.gaSelected = true;
        console.log('[Google Integration] Auto-selected GA property:', match.name);
      }
    } catch (err) {
      console.warn('[Google Integration] GA auto-select failed:', err.message);
    }
  }
...
export async function fetchGSCQueriesForAIPages(accessToken, siteUrl, daysOrRange = 30, aiPagePaths = []) {
  ...
  // ── Derive a base URL for building page-filter expressions ──
  // GSC properties come in two flavors:
  //   • URL-prefix property: gscSiteUrl looks like "https://example.com/"
  //   • Domain property:     gscSiteUrl looks like "sc-domain:example.com"
  // For the page filter we need a real http(s) URL to compare against the
  // page dimension that GSC returns.
  let baseHost; // the bare host, e.g. "example.com"
  if (siteUrl.startsWith('sc-domain:')) {
    baseHost = siteUrl.slice('sc-domain:'.length).replace(/\/$/, '');
  } else {
    try {
      baseHost = new URL(siteUrl).host;
    } catch {
      baseHost = siteUrl.replace(/^https?:\/\//, '').replace(/\/$/, '');
    }
  }

  // For each AI page path, build a small set of URL variants we will look up
  // via GSC's page filter. GSC stores the canonical URL it indexed, which
  // can differ from what GA4 returns by trailing slash, www subdomain, or
  // protocol - so we try the common combinations.
  const variantCount = 4; // [with/without trailing slash] x [with/without www]
  const buildVariants = (path) => {
    let p = path;
    let absolute = null;
    ...
    let pathOnly = absolute ? new URL(absolute).pathname : p;
    // Strip query string / hash for the filter so we match indexed URLs
    pathOnly = pathOnly.split('?')[0].split('#')[0];
    const noSlash = pathOnly.replace(/\/$/, '') || '/';
    const withSlash = noSlash === '/' ? '/' : noSlash + '/';
    const hostBare = baseHost.replace(/^www\./, '');
    const hostWww = 'www.' + hostBare;
    return [
      `https://${hostBare}${noSlash}`,
      `https://${hostBare}${withSlash}`,
      `https://${hostWww}${noSlash}`,
      `https://${hostWww}${withSlash}`,
    ];
  };

  const pages = aiPagePaths.slice(0, 10); // cap to top 10 AI pages
  // Flat list of every URL variant; one filterGroup per variant. GSC ORs
  // groups together so a row matches if ANY variant matches.
  const allVariants = pages.flatMap(buildVariants);
  const dimensionFilterGroups = allVariants.map(u => ({
    filters: [{ dimension: 'page', operator: 'equals', expression: u }],
  }));
  ...
  try {
    ...
    // Aggregate by query (the same query can fire on multiple variants/pages).
    // ctrSum/count tracks a session-weight-free average; we'll recompute
    // a clicks/impressions-weighted CTR on output for accuracy.
    const queryMap = {};
    for (const row of rows) {
      ...
      if (!queryMap[query]) {
        queryMap[query] = {
          query,
          page,
          pageClicks: clicks, // tracks the best-page choice
          clicks: 0,
          impressions: 0,
        };
      }
      ...
      // Keep the page that earned the most clicks under this query
      if (clicks > queryMap[query].pageClicks) {
        queryMap[query].pageClicks = clicks;
    ...
    return Object.values(queryMap)
      .map(q => ({
        ...
        // True weighted CTR = total clicks / total impressions
        ctr: q.impressions ? ((q.clicks / q.impressions) * 100).toFixed(1) : '0.0',
      }))
...
export async function fetchAITrafficStats(accessToken, propertyId, daysOrRange = 30, compareRange = null) {
  ...
  // ── 7. AI engagement totals (current period, single-row aggregate) ──
  // averageSessionDuration is a ratio metric - must NOT be summed across rows.
  // Run it dimension-less so GA4 returns the correctly-weighted aggregate.
  const aiTotalsBody = {
    dateRanges: [{ startDate, endDate }],
    metrics: [
      { name: 'sessions' },
  ...
  // ── 8. Previous-period AI engagement totals (for trend badges) ──
  const prevAiTotalsBody = {
    dateRanges: [{ startDate: compStart, endDate: compEnd }],
    metrics: [
      { name: 'sessions' },
      { name: 'engagedSessions' },
  ...
  // ── 9. AI sessions per day (current period) ──
  const byDateBody = {
    dateRanges: [{ startDate, endDate }],
  ...
  // ── 10. AI sessions per day per engine (for stacked timeseries) ──
  const byDateSourceBody = {
    dateRanges: [{ startDate, endDate }],
  ...
  const [
    bySourceRes, byPageRes, totalRes, prevAiRes, prevTotalRes, bySourcePageRes,
    aiTotalsRes, prevAiTotalsRes, byDateRes, byDateSourceRes,
  ] = await Promise.all([
    ...
    fetch(url, { method: 'POST', headers, body: JSON.stringify(aiTotalsBody) }).catch(() => null),
    fetch(url, { method: 'POST', headers, body: JSON.stringify(prevAiTotalsBody) }).catch(() => null),
    fetch(url, { method: 'POST', headers, body: JSON.stringify(byDateBody) }).catch(() => null),
    fetch(url, { method: 'POST', headers, body: JSON.stringify(byDateSourceBody) }).catch(() => null),
  ]);
  ...
  const [bySourceData, byPageData, totalData, bySourcePageData, aiTotalsData, byDateData, byDateSourceData] = await Promise.all([
    bySourceRes.json(),
    byPageRes.ok ? byPageRes.json() : { rows: [] },
    totalRes.ok ? totalRes.json() : { rows: [] },
    bySourcePageRes?.ok ? bySourcePageRes.json() : { rows: [] },
    aiTotalsRes?.ok ? aiTotalsRes.json() : { rows: [] },
    byDateRes?.ok ? byDateRes.json() : { rows: [] },
    byDateSourceRes?.ok ? byDateSourceRes.json() : { rows: [] },
  ]);

  // Parse previous-period data
  let prevAiSessions = 0;
  let prevTotalSessions = 0;
  let prevAiEngagedSessions = 0;
  let prevAiAvgDuration = 0;
  let prevAiKeyEvents = 0;
  try {
    if (prevAiRes?.ok) {
  ...
  try {
    if (prevAiTotalsRes?.ok) {
      const d = await prevAiTotalsRes.json();
      const mv = d.rows?.[0]?.metricValues || [];
      // index order matches prevAiTotalsBody.metrics
      prevAiEngagedSessions = Number(mv[1]?.value || 0);
      prevAiAvgDuration = Number(mv[2]?.value || 0);
      prevAiKeyEvents = Number(mv[3]?.value || 0);
    }
  } catch { /* ignore */ }
  ...
  // ── AI engagement aggregates (from dedicated dimension-less query) ──
  // We use the aiTotals row (not summing bySource rows) because
  // averageSessionDuration is a ratio metric and cannot be summed.
  let aiEngagedSessions = 0;
  let aiAvgDuration = 0;
  let aiKeyEvents = 0;
  let aiTotalsSessions = 0; // sanity-check value, should equal totalAiSessions
  try {
    const mv = aiTotalsData.rows?.[0]?.metricValues || [];
    aiTotalsSessions = Number(mv[0]?.value || 0);
    aiEngagedSessions = Number(mv[1]?.value || 0);
    aiAvgDuration = Number(mv[2]?.value || 0);
    aiKeyEvents = Number(mv[3]?.value || 0);
  } catch { /* ignore */ }

  // If GA4 returns slightly different totals between the source-grouped query
  // and the dimension-less query (rare, but possible due to sampling on huge
  // properties), trust the dimension-less aggregate for engagement metrics
  // and keep the source-grouped sum for the engine breakdown - both are
  // internally consistent within their own slice of the UI.

  // ── Daily timeseries (total AI sessions per day) ──
  // Build a complete date sequence so the chart is not gappy on zero-traffic days.
  const dayList = [];
  {
    ...
    for (let d = new Date(s); d <= e; d.setUTCDate(d.getUTCDate() + 1)) {
      ...
      dayList.push(`${y}${m}${dd}`); // GA4 returns YYYYMMDD
    }
  }
  const dailySessionsMap = {};
  for (const row of (byDateData.rows || [])) {
    const date = row.dimensionValues[0].value;
    const sessions = Number(row.metricValues[0].value);
    dailySessionsMap[date] = sessions;
  }
  // Per-engine per-day: { date: { engineName: sessions } }
  const dailyEngineMap = {};
  for (const row of (byDateSourceData.rows || [])) {
    const date = row.dimensionValues[0].value;
    const rawSource = row.dimensionValues[1].value;
    const sessions = Number(row.metricValues[0].value);
    const engine = classifyAiSource(rawSource);
    if (!dailyEngineMap[date]) dailyEngineMap[date] = {};
    dailyEngineMap[date][engine] = (dailyEngineMap[date][engine] || 0) + sessions;
  }
  const dailyTimeseries = dayList.map(date => {
    const perEngine = dailyEngineMap[date] || {};
    return {
      date,
      sessions: dailySessionsMap[date] || 0,
      byEngine: perEngine,
    };
  });
  ...
  // Engagement rate = engagedSessions / sessions, in % with 1 decimal.
  // Use aiTotalsSessions as the denominator (same query as the numerator) to
  // avoid mixing data sources.
  const aiEngagementRate = aiTotalsSessions
    ? parseFloat(((aiEngagedSessions / aiTotalsSessions) * 100).toFixed(1))
    : 0;
  const prevAiEngagementRate = prevAiSessions
    ? parseFloat(((prevAiEngagedSessions / prevAiSessions) * 100).toFixed(1))
    : 0;
  ...
  return {
    ...
    // New engagement quality metrics for AI traffic
    aiEngagedSessions,
    aiEngagedSessionsChange: pct(aiEngagedSessions, prevAiEngagedSessions),
    aiEngagementRate,
    aiEngagementRateChange: prevAiEngagementRate
      ? Math.round((aiEngagementRate - prevAiEngagementRate) * 10) / 10
      : null,
    aiAvgDuration,
    aiAvgDurationChange: pct(aiAvgDuration, prevAiAvgDuration),
    aiKeyEvents,
    aiKeyEventsChange: pct(aiKeyEvents, prevAiKeyEvents),

    // Daily timeseries (one row per day in the range)
    dailyTimeseries,
