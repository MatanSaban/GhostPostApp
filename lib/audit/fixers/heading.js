...
export async function preview({ site, payload = {}, wpAuto }) {
  ...
  if (issueType === 'audit.issues.multipleH1') {
    return previewMultipleH1({ issues: allIssues }, requestedUrls, wpAuto);
  }
  ...
  const issues = allIssues.filter((i) => i.message === issueType);
  ...
  const pageResults = auditId ? await getAllPageResults(auditId) : [];
