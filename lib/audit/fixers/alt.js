...
export async function preview({ site, payload = {}, wpAuto }) {
  ...
  const allIssues = auditId ? await getAllIssues(auditId) : [];
  const altIssues = allIssues.filter((i) => i.message === 'audit.issues.imagesNoAlt');
  ...
  const altPageResults = auditId ? await getAllPageResults(auditId) : [];
  const sampleTitles = altPageResults.map((p) => p.title)
