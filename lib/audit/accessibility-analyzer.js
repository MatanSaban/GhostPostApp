...
export async function analyzeAccessibility(page, pageUrl, options = {}) {
  ...
  if (!axeResults?.violations?.length && !axeResults?.passes?.length) {
    console.log(`[A11y] No violations or passes found on ${pageUrl}`);
    return issues;
  }

  // ── Passed rules → "passed" severity issues ───────────────
  if (axeResults.passes?.length) {
    for (const rule of axeResults.passes) {
      issues.push({
        type: 'accessibility',
        severity: 'passed',
        message: `a11y.${rule.id}`,
        url: pageUrl,
        suggestion: rule.help || '',
        source: 'axe',
    ...
    console.log(`[A11y] ${axeResults.passes.length} rules passed on ${pageUrl}`);
  }
  ...
  const passedCount = axeResults.passes?.length || 0;
  const violationCount = axeResults.violations?.length || 0;
  console.log(
    `[A11y] Produced ${issues.length} accessibility issues (${passedCount} passed, ${violationCount} violations), ${screenshotsTaken} element screenshots`
  );
