 ...
 */
export function analyzeHtml(html, pageUrl, headers = {}, ttfb = null, domData = null, imageResources = null) {
  const issues = [];
  ...
  if (images.length > 0) {
    const oldFormatExts = /\.(jpe?g|png|gif|bmp|tiff?)$/i;
    const pictureElements = $('picture');
    const hasSourceWebpAvif = pictureElements.find('source[type="image/webp"], source[type="image/avif"]').length > 0;
    const oldFormatImages = [];
    images.each((_, el) => {
      const src = $(el).attr('src') || '';
      // Skip data URIs, SVGs, and already next-gen formats
      if (!src || src.startsWith('data:') || /\.(svg|webp|avif)$/i.test(src)) return;
      if (oldFormatExts.test(src.split('?')[0])) {
        // Check if this img is inside a <picture> with next-gen sources
        const parent = $(el).parent('picture');
        if (parent.length && parent.find('source[type="image/webp"], source[type="image/avif"]').length > 0) return;
        oldFormatImages.push({ url: src, fileName: src.split('/').pop()?.split('?')[0] || '' });
      }
    });
    if (oldFormatImages.length > 3) {
      issues.push({ type: 'performance', severity: 'warning', message: 'audit.issues.imagesNotNextGen', url: pageUrl, suggestion: 'audit.suggestions.useNextGenFormats', source: 'html', details: `${oldFormatImages.length}/${images.length}`, detailedSources: oldFormatImages.slice(0, 15) });
    } else if (oldFormatImages.length === 0) {
      issues.push({ type: 'performance', severity: 'passed', message: 'audit.issues.imagesNextGenGood', url: pageUrl, source: 'html' });
    }
  }

  // ── Image File Sizes ──
  if (imageResources && Object.keys(imageResources).length > 0) {
    const WARN_THRESHOLD = 200 * 1024;  // 200 KB
    const ERROR_THRESHOLD = 500 * 1024; // 500 KB
    const largeImages = [];
    for (const [imgUrl, info] of Object.entries(imageResources)) {
      if (info.size > WARN_THRESHOLD) {
        const sizeKB = Math.round(info.size / 1024);
        largeImages.push({ url: imgUrl, fileName: imgUrl.split('/').pop()?.split('?')[0] || '', size: `${sizeKB}KB` });
      }
    }
    const criticalCount = Object.values(imageResources).filter(i => i.size > ERROR_THRESHOLD).length;
    if (criticalCount > 0) {
      issues.push({ type: 'performance', severity: 'error', message: 'audit.issues.imagesTooLarge', url: pageUrl, suggestion: 'audit.suggestions.optimizeImageSize', source: 'html', details: `${criticalCount} images over 500KB`, detailedSources: largeImages.slice(0, 15) });
    } else if (largeImages.length > 0) {
      issues.push({ type: 'performance', severity: 'warning', message: 'audit.issues.imagesLargeWarning', url: pageUrl, suggestion: 'audit.suggestions.optimizeImageSize', source: 'html', details: `${largeImages.length} images over 200KB`, detailedSources: largeImages.slice(0, 15) });
    } else {
      issues.push({ type: 'performance', severity: 'passed', message: 'audit.issues.imagesSizeGood', url: pageUrl, source: 'html' });
    }
  }
  ...
  // ── Body Content (empty / thin content) ──
  // Strip chrome elements that exist on every page (nav, header, footer,
  // sidebars, scripts) and measure the words left in the main content area.
  // Pages with virtually no body text show up here as `emptyContent` so the
  // AI agent can offer to generate body copy. Threshold is intentionally
  // conservative: only flags pages that are essentially blank.
  try {
    const $body = $('body').clone();
    $body.find('script, style, noscript, template, nav, header, footer, aside, [role="navigation"], [role="banner"], [role="contentinfo"], .menu, .navigation, .site-header, .site-footer, .widget, .sidebar').remove();
    ...
    if (wordCount < 50) {
      issues.push({
        type: 'technical',
        severity: 'error',
        message: 'audit.issues.emptyContent',
        url: pageUrl,
        suggestion: 'audit.suggestions.fillEmptyContent',
        source: 'html',
  ...
  } catch { /* skip if body extraction fails */ }
  ...
  // ── H2 Subheadings ──
  // Prefer live DOM h2Count (from Playwright evaluate) over Cheerio, since
  // Cheerio may miss elements rendered by client-side JS.
  const h2Count = domData?.h2Count ?? $('h2').length;
  if (h2Count === 0) {
  ...
  const googlebotContent = ($('meta[name="googlebot"]').attr('content') || '').toLowerCase();
  const xRobotsTag = (headers['x-robots-tag'] || '').toLowerCase();

  const hasNoindex = robotsContent.includes('noindex') || googlebotContent.includes('noindex') || xRobotsTag.includes('noindex');
  const hasNofollow = robotsContent.includes('nofollow') || googlebotContent.includes('nofollow') || xRobotsTag.includes('nofollow');

  if (hasNoindex) {
    const source_details = [];
    if (robotsContent.includes('noindex')) source_details.push('meta robots');
    if (googlebotContent.includes('noindex')) source_details.push('meta googlebot');
    if (xRobotsTag.includes('noindex')) source_details.push('X-Robots-Tag header');
    issues.push({ type: 'technical', severity: 'warning', message: 'audit.issues.metaRobotsNoindex', url: pageUrl, suggestion: 'audit.suggestions.removeNoindex', source: 'html', details: source_details.join(', ') });
  }
  if (hasNofollow) {
    const source_details = [];
    if (robotsContent.includes('nofollow')) source_details.push('meta robots');
    if (googlebotContent.includes('nofollow')) source_details.push('meta googlebot');
    if (xRobotsTag.includes('nofollow')) source_details.push('X-Robots-Tag header');
    issues.push({ type: 'technical', severity: 'warning', message: 'audit.issues.metaRobotsNofollow', url: pageUrl, suggestion: 'audit.suggestions.removeNofollow', source: 'html', details: source_details.join(', ') });
  }
  if (!hasNoindex && !hasNofollow) {
    issues.push({ type: 'technical', severity: 'passed', message: 'audit.issues.metaRobotsGood', url: pageUrl, source: 'html' });
  }
  ...
  if (!ogTitle || !ogDesc || !ogImage) {
    ...
    issues.push({ type: 'technical', severity: 'warning', message: 'audit.issues.missingOG', url: pageUrl, suggestion: 'audit.suggestions.addOG', source: 'html', details: `Missing: ${missing.join(', ')}` });
  } else {
  ...
  // ── Viewport Meta Tag ──
  // Required for mobile rendering; its absence forces desktop layout on
  // phones and tanks the mobile Lighthouse score.
  const viewportMeta = $('meta[name="viewport"]').attr('content');
  if (!viewportMeta) {
    issues.push({ type: 'technical', severity: 'error', message: 'audit.issues.noViewportMeta', url: pageUrl, suggestion: 'audit.suggestions.addViewportMeta', source: 'html' });
  } else if (!/width\s*=\s*device-width/i.test(viewportMeta)) {
    issues.push({ type: 'technical', severity: 'warning', message: 'audit.issues.viewportMetaWeak', url: pageUrl, suggestion: 'audit.suggestions.addViewportMeta', source: 'html', details: viewportMeta.slice(0, 120) });
  } else {
    issues.push({ type: 'technical', severity: 'passed', message: 'audit.issues.viewportMetaGood', url: pageUrl, source: 'html' });
  }

  // ── Charset Declaration ──
  // Must appear in first 1024 bytes to be honored by browsers.
  const charsetMeta = $('meta[charset]').attr('charset') ||
    ($('meta[http-equiv="Content-Type"]').attr('content') || '').match(/charset=([\w-]+)/i)?.[1];
  if (!charsetMeta) {
    issues.push({ type: 'technical', severity: 'warning', message: 'audit.issues.noCharset', url: pageUrl, suggestion: 'audit.suggestions.addCharset', source: 'html' });
  } else {
    issues.push({ type: 'technical', severity: 'passed', message: 'audit.issues.charsetGood', url: pageUrl, source: 'html', details: charsetMeta });
  }
  ...
  if (hasHeaders) {
    ...
    // Mixed Content - only checks subresources the browser would actually
    // block / warn on. Outbound <a href="http://..."> links are normal
    // external links and do not constitute mixed content.
    if (pageUrl.startsWith('https://')) {
      const MIXED_CONTENT_SELECTOR =
        'img[src], script[src], iframe[src], video[src], audio[src], source[src], track[src], embed[src], object[data], link[href][rel~="stylesheet"], link[href][rel~="preload"]';
      let mixedCount = 0;
      $(MIXED_CONTENT_SELECTOR).each((_, el) => {
        const attr = $(el).attr('src') || $(el).attr('href') || $(el).attr('data') || '';
  ...
  try {
    ...
    if (res.ok) {
      ...
      for (const line of lines) {
        ...
        } else if (line.startsWith('disallow:') && (currentAgent === '*' || currentAgent === '')) {
          const path = line.replace('disallow:', '').trim();
          if (path === '/') blocksAll = true;
        }
      }
      if (blocksAll) {
        issues.push({ type: 'technical', severity: 'error', message: 'audit.issues.robotsTxtBlocksAll', suggestion: 'audit.suggestions.fixRobotsTxt', source: 'html' });
      }
    } else {
