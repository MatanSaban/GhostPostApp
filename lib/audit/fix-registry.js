...
export const ISSUE_FIXERS = {
  ...
  'audit.issues.emptyContent': {
    // Drafts a full HTML body for blank pages/posts. WP-auto pushes via
    // updatePost { content: html }; non-WP returns the HTML as a snippet
    // for paste-in. 8 credits matches favicon - it's the most expensive
    // text generation we run (full body, not a single field).
    kind: 'ai', handler: 'emptyContent', credits: 8, bulk: true, previewable: true,
    manualKinds: ['snippet'],
  },
