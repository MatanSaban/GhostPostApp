...
export async function processChunk(auditId, options = {}) {
  ...
  try {
    ...
    const persistPage = async (pageUrl, pageIssues, pageResult) => {
      // Three attempts. First failure is usually a P2034 (write conflict
      // racing the lease refresh — rare since $push doesn't read first, but
      // possible) or P2010 (transient pool issue). Atomic ops mean we don't
      // need the dbWrite read-modify-write retry pattern; just retry the op.
      ...
      try {
        ...
        if (retryScanner) {
          ...
          if (successful.length > 0) {
            ...
            // Apply updates by reading-modify-writing the full pageResults
            // array. Cheap (one read, one write) and we're already at the
            // tail of the audit, so contention is minimal.
            ...
            await dbWrite(async () => {
              ...
              const updated = (fresh.pageResults || []).map((pr) => {
                ...
                if (!u) return pr;
                return { ...pr, screenshotDesktop: u.screenshotDesktop, screenshotMobile: u.screenshotMobile };
              });
              await prisma.siteAudit.update({
                where: { id: auditId },
