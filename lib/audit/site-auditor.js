...
import {
  ...
  applyBulkUpdates,
  getAllPageResults,
  shouldIncludePageResultInRawPush,
} from './page-results-helper.js';
import {
  appendIssues,
  getAllIssues,
  replaceAllIssues,
  shouldIncludeIssuesInRawPush,
  findDuplicateFields,
} from './issues-helper.js';
  ...
  try {
    ...
    await dbWrite(() =>
      prisma.siteAudit.update({
        ...
        siteWideIndicators: {
          hasSiteWideNoindex: deduped.some(
            (i) => i.message === 'audit.issues.wpSearchEngineDiscouraged' && i.severity !== 'passed'
          ),
          missingSecurityHeaders: Array.from(new Set(
            deduped
              .filter((i) => (
                i.message === 'audit.issues.noHsts'
                || i.message === 'audit.issues.noXFrameOptions'
                || i.message === 'audit.issues.noContentTypeOptions'
                || i.message === 'audit.issues.noCsp'
                || i.message === 'audit.issues.noReferrerPolicy'
                || i.message === 'audit.issues.noPermissionsPolicy'
              ) && i.severity !== 'passed')
              .map((i) => i.message)
          )),
        },
...
// Capped audit-level error log. Each call atomically appends one entry to
// `chunkErrors` and trims the array to the most recent 50 — so a chronically
// failing page can't bloat the document. Failures here are swallowed: an
// error logger that itself errors must not cascade.
async function logAuditError(auditId, kind, message, extra = {}) {
  if (!auditId) return;
  try {
          ...
          $push: {
            chunkErrors: {
              $each: [{
                ...
                kind: String(kind || 'unknown').slice(0, 50),
    ...
    // Logger failures are deliberately silent.
  }
}
...
export async function runDiscovery(auditId, siteUrl, siteId, deviceType, options = {}) {
  ...
  try {
    ...
    if (!urls || urls.length === 0) {
      ...
      // Phase 2: write the seed issue via helper (covers both embedded + collection
      // depending on flag mode). Site doc lookup so we have the accountId.
      const siteRow = await prisma.site.findUnique({
        where: { id: siteId },
        select: { accountId: true },
      });
      await replaceAllIssues(
        { auditId, siteId, accountId: siteRow?.accountId },
        [noUrlsIssue],
      ).catch((err) => {
        console.warn(`[runDiscovery] NO_URLS issue write failed: ${err.message}`);
      });
      invalidateAudit(siteId);
    ...
    await dbWrite(() =>
      prisma.siteAudit.update({
        ...
        data: {
          ...
          discoveryMethod: method,
          deviceType: deviceType || null,
          // Phase 2: site-wide issues (currently just noSitemap) seeded via
          // helper below so they land in both stores per flag.
          issues: [],
          progress: {
            currentStep: 1,
            totalSteps: pagesToScan.length + 3,
    ...
    // Phase 2: seed site-wide issues via helper (covers both stores).
    if (siteWideIssues.length > 0) {
      const siteRow2 = await prisma.site.findUnique({
        where: { id: siteId },
        select: { accountId: true },
      });
      await appendIssues(
        { auditId, siteId, accountId: siteRow2?.accountId },
        siteWideIssues,
      ).catch((err) => {
        console.warn(`[runDiscovery] siteWideIssues append failed: ${err.message}`);
      });
    }
...
export async function processChunk(auditId, options = {}) {
  ...
  const LEASE_MS = 5 * 60 * 1000;        // chunk hard cap before another worker can steal it
  const LEASE_REFRESH_MS = 30 * 1000;    // periodic refresh while the chunk is alive

  // ── Atomic lease acquire ──────────────────────────────────────────
  // Conditional updateMany serves as compare-and-swap. We grab the lease only
  // if the audit is in the right state AND nobody else holds an unexpired
  // lease. updateMany returns the count of matched docs — 0 means somebody
  // beat us to it (or the audit isn't scannable). This makes concurrent chunks
  // impossible by construction even if heartbeat-staleness fakes out the
  // watchdog and it re-fires /continue against a chunk that's still alive.
  const leaseExpiresAt = new Date(Date.now() + LEASE_MS);
  const acquired = await prisma.siteAudit.updateMany({
    where: {
      id: auditId,
      status: 'RUNNING',
      phase: 'scanning',
      OR: [
        ...
        { chunkLeaseUntil: { lt: new Date() } },
      ],
    },
    data: { chunkLeaseUntil: leaseExpiresAt },
  });
  if (acquired.count === 0) {
    return { ok: false, reason: 'lease-held-or-bad-state' };
  }

  // ── Snapshot post-acquire ─────────────────────────────────────────
  // Now that we hold the lease, re-read what we need. The state we care about
  // (pendingUrls, pagesScanned) can only change via another lease-holder, and
  // that's us — so the read here is consistent for the chunk's lifetime.
  ...
  if (!audit) {
    // Belt-and-suspenders. acquired.count > 0 already proved the audit
    // exists, but if it just got force-deleted between the two queries,
    // release the lease via best-effort and exit.
    return { ok: false, reason: 'audit-not-found' };
  }

  const pending = audit.pendingUrls || [];
  if (pending.length === 0) {
    // Nothing to do — release the lease so the next caller can advance the
    // pipeline (it'll see empty queue and trigger /finalize).
    await prisma.siteAudit.update({
      where: { id: auditId },
      data: { chunkLeaseUntil: null },
    }).catch(() => {});
  ...
  // ── Lease refresh timer ───────────────────────────────────────────
  // Bumps the lease forward periodically while the chunk runs. Catches the
  // case where a slow chunk (PSI taking 60s, big page) outlives the initial
  // 5-min lease — without this, the watchdog could legitimately re-fire.
  const refreshTimer = setInterval(() => {
    prisma.siteAudit.update({
      where: { id: auditId },
      data: { chunkLeaseUntil: new Date(Date.now() + LEASE_MS) },
    }).catch(() => {}); // best-effort; if it fails, next tick or chunk-end will refresh
  }, LEASE_REFRESH_MS);
  refreshTimer.unref?.(); // don't hold the event loop open if Node tries to exit
  ...
  try {
    ...
    // Per-page atomic persist via $runCommandRaw with $push/$pull/$inc.
    // Each page commits its own issues + pageResult independently of the
    // chunk's other pages — so a failure on page 17 doesn't lose pages 1-16
    // of work. Replaces the old chunk-end commit (which threw away the whole
    // chunk if its single read-modify-write failed permanently).
    //
    // Why $runCommandRaw and not Prisma's typed update():
    //   • $push/$pull/$inc are atomic and don't need a read-modify-write,
    //     so they don't conflict with the lease-refresh + heartbeat writers.
    //   • Avoids the 1MB+ document re-read per page that the typed update
    //     would force (issues + pageResults grow throughout the audit).
    const persistPage = async (pageUrl, pageIssues, pageResult) => {
      // Phase 1 dual-write awareness:
      //   - When the helper says embedded writes are still on (modes 'off' or
      //     'dual'), keep pageResults inside the same atomic raw $push so
      //     issues + pageResult + pendingUrl + pagesScanned all land or none
      //     do. Atomicity story unchanged from pre-Phase-1.
      //   - When the helper says embedded writes are off (mode 'on'), drop
      //     pageResults from the raw $push and rely on the AuditPageResultDoc
      //     write below. Slightly weaker atomicity (the doc insert lives in a
      //     separate op) but simpler — and the watchdog's worst case is one
      //     orphan issue without its pageResult, which is harmless.
      // Phase 1+2 dual-write awareness: each helper decides whether the
      // embedded array gets the $push, independently. The atomic raw command
      // includes whichever embedded fields are still being written; per-doc
      // collection inserts run after.
      const includeEmbeddedPageResult = shouldIncludePageResultInRawPush();
      const includeEmbeddedIssues = shouldIncludeIssuesInRawPush();
      const pushOps = {};
      if (includeEmbeddedIssues) pushOps.issues = { $each: pageIssues };
      if (includeEmbeddedPageResult) pushOps.pageResults = { $each: [pageResult] };

      const rawUpdate = {
        $pull: { pendingUrls: pageUrl },
        $inc: { pagesScanned: 1 },
        $currentDate: { updatedAt: true },
      };
      if (Object.keys(pushOps).length > 0) rawUpdate.$push = pushOps;

      for (let attempt = 1; attempt <= 3; attempt++) {
        try {
          await prisma.$runCommandRaw({
            update: 'SiteAudit',
            updates: [{ q: { _id: { $oid: auditId } }, u: rawUpdate }],
          });
          break;
        } catch (err) {
          if (attempt === 3) {
            // Three retries failed. The page's URL stays in pendingUrls so
            // the next chunk will re-scan it. Cloudinary uploads are wasted
            // but no data corruption — we just lose this page's work.
            console.warn(`[SiteAudit/chunked] Per-page persist failed for ${pageUrl}: ${err.message}`);
            logAuditError(auditId, 'persist-failed', err.message, { url: pageUrl });
            return false;
          }
      ...
      // Collection inserts. Both helpers see skipEmbedded:true since the raw
      // command above already handled the embedded side when applicable.
      try {
        await appendPageResult(
          { auditId, siteId: audit.siteId, accountId: site.accountId },
          pageResult,
          { skipEmbedded: true },
        );
      } catch (err) {
        console.warn(`[SiteAudit/chunked] AuditPageResult insert failed for ${pageUrl}: ${err.message}`);
        logAuditError(auditId, 'page-result-doc-failed', err.message, { url: pageUrl });
      }
      ...
      try {
        ...
        console.warn(`[SiteAudit/chunked] AuditIssue insert failed for ${pageUrl}: ${err.message}`);
        logAuditError(auditId, 'issue-doc-failed', err.message, { url: pageUrl });
      }
      return true;
    };
    ...
    // Throttle per-page heartbeats. Three pages finishing within milliseconds
    // of each other would otherwise produce 3 concurrent writes to the same
    // document and trip MongoDB's write-conflict path. Coalescing to at most
    // one heartbeat every ~2s keeps the watchdog informed without the spam.
    ...
    const scanTasks = batch.map((pageUrl, idx) =>
      limit(async () => {
        ...
        try {
          ...
          await persistPage(pageUrl, result.issues || [], pageResult);
        } catch (err) {
          console.warn(`[SiteAudit/chunked] Page scan failed for ${pageUrl}:`, err.message);
          logAuditError(auditId, 'page-scan-failed', err.message, { url: pageUrl });
          // Persist a synthetic "page load failed" issue so the URL doesn't
          // get re-scanned next chunk (we still $pull it from pendingUrls)
          // and the user sees the failure surface in the issues list.
          await persistPage(pageUrl, [{
            type: 'technical', severity: 'error',
            message: 'audit.issues.pageLoadFailed',
            url: pageUrl, suggestion: 'audit.suggestions.checkUrl', source: 'system',
          }], {
            url: pageUrl, statusCode: null, title: null, metaDescription: null,
            ttfb: null, performanceScore: null, lcp: null, cls: null, inp: null,
            jsErrors: [], brokenResources: [], issueCount: 1,
            screenshotDesktop: null, screenshotMobile: null,
            screenshotsDesktop: [], screenshotsMobile: [],
            filmstripDesktop: null, filmstripMobile: null,
          });
        }

        scannedInChunk++;

        // Per-page heartbeat — but throttled. The watchdog only cares about
        // gaps >5min, the UI poll runs every 3s; updating the doc faster than
        // every ~2s just causes write conflicts when concurrent pages race.
        // Always emit on the very first and very last page of the chunk so
        // pagesScanned + URL label stay accurate at the boundaries.
        const totalScanned = baseScanned + scannedInChunk;
        const totalPages = (audit.pagesFound || batch.length);
        const isFirst = scannedInChunk === 1;
        const isLast = scannedInChunk === batch.length;
        const sinceLast = Date.now() - lastHeartbeatAt;
        if (isFirst || isLast || sinceLast > 2000) {
          ...
          // Heartbeat now writes ONLY the progress label + percentage. The
          // pagesScanned counter is owned by persistPage's $inc — racing it
          // here would cause drift. Two-attempt retry keeps the UI smooth
          // through transient write conflicts; failure is logged but not
          // thrown (lease-refresh keeps updatedAt alive independently).
          (async () => {
            for (let attempt = 1; attempt <= 2; attempt++) {
              try {
                ...
                if (attempt === 2) {
                  console.warn(`[SiteAudit/chunked] Heartbeat dropped for ${auditId} page ${totalScanned}: ${err.message}`);
                  return;
                }
    ...
    // ── Release lease ──────────────────────────────────────────────────
    // Per-page persist already wrote everyone's issues + pageResults and
    // drained their URLs from pendingUrls atomically. All we need to do at
    // chunk-end is hand the baton: clear the lease so the next /continue
    // (or /finalize, if pendingUrls is now empty) can run.
    await dbWrite(() =>
      prisma.siteAudit.update({
        where: { id: auditId },
        data: { chunkLeaseUntil: null },
    ...
    // Re-read pendingUrls fresh — pages may have failed to persist (in which
    // case they remain in the queue) so hasMore is the truth from the DB,
    // not our local "remainingAfter" computation.
    const after = await prisma.siteAudit.findUnique({
      where: { id: auditId },
      select: { pendingUrls: true },
    });
    const pendingLeft = after?.pendingUrls?.length || 0;

    return { ok: true, hasMore: pendingLeft > 0, processed: batch.length };
  } catch (error) {
    console.error(`[SiteAudit/chunked] processChunk error for ${auditId}:`, error);
    // Don't leak the lease on error. Best-effort clear so the watchdog can
    // re-fire /continue against this audit instead of waiting 5min for the
...
export async function runFinalization(auditId, options = {}) {
  ...
  try {
    ...
    audit = await prisma.siteAudit.findUnique({
      where: { id: auditId },
      select: {
        id: true, siteId: true, deviceType: true, pagesScanned: true,
      },
    });
    ...
    // Phase 2: issues read via helper (collection or embedded fallback).
    // Phase 1: pageResults same shape.
    const allIssues = [...(await getAllIssues(auditId))];
    const pageResults = await getAllPageResults(auditId);

    // ── Duplicate Titles / Meta Descriptions (cross-page) ────────────
    // Phase 4: helper-driven. Same logic, centralized — eventually backed by
    // a server-side $group when AuditPageResult moves to a fully-indexed
    // read path (currently still loads all pageResults, but the win is the
    // single function call + future-proof shape).
    if (pageResults.length > 1) {
      const titleDupes = await findDuplicateFields(auditId, 'title');
      const descDupes = await findDuplicateFields(auditId, 'metaDescription');
      for (const urls of titleDupes.values()) {
        for (const pageUrl of urls) {
          allIssues.push({
            type: 'technical', severity: 'warning',
            message: 'audit.issues.duplicateTitle',
      ...
      for (const urls of descDupes.values()) {
        for (const pageUrl of urls) {
          allIssues.push({
            type: 'technical', severity: 'warning',
            message: 'audit.issues.duplicateMetaDescription',
    ...
    // ── Recover missing screenshots ────────────────────────────────────
    // During the chunked scan, Cloudinary uploads can fail under load — when
    // they do, the page's screenshotDesktop/Mobile become null but the scan
    // itself succeeded (statusCode 2xx). Try to backfill the homepage + the
    // first few pages so the top-level `screenshots` field and AI Vision
    // have something to work with. Cap aggressively (10 pages) to keep
    // finalize fast — backfilling 200 missing screenshots would be punishing.
    const MAX_SCREENSHOT_RETRIES = 10;
    const missing = pageResults.filter((pr) => {
      const ok = pr.statusCode && pr.statusCode >= 200 && pr.statusCode < 400;
      return ok && !pr.screenshotDesktop && !pr.screenshotMobile;
    }).slice(0, MAX_SCREENSHOT_RETRIES);

    if (missing.length > 0) {
      console.log(`[SiteAudit/chunked] Retrying ${missing.length} missing screenshots for ${auditId}`);
      let retryScanner = null;
      try {
        retryScanner = await createBrowserScanner();
        if (retryScanner) {
          const retryFolder = await resolveAuditFolder(auditId, audit.siteId, site.url);
          const safeUpload = (p) => p.catch(() => null);
          const recovered = await Promise.all(missing.map(async (pr) => {
            try {
              const r = await scanPage(retryScanner, pr.url, {
                captureScreenshots: true,
                deviceType: audit.deviceType,
                runAccessibility: false,
              });
              const ss = r.screenshots || {};
              const [d, m] = await Promise.all([
                safeUpload(uploadPageScreenshot(ss.desktop, retryFolder, pr.url, 'desktop')),
                safeUpload(uploadPageScreenshot(ss.mobile, retryFolder, pr.url, 'mobile')),
              ]);
              return { url: pr.url, screenshotDesktop: d || null, screenshotMobile: m || null };
            } catch (err) {
              console.warn(`[SiteAudit/chunked] Screenshot retry failed for ${pr.url}: ${err.message}`);
              logAuditError(auditId, 'screenshot-retry-failed', err.message, { url: pr.url });
              return null;
            }
          }));

          const successful = recovered.filter((u) => u && (u.screenshotDesktop || u.screenshotMobile));
          if (successful.length > 0) {
            console.log(`[SiteAudit/chunked] Recovered ${successful.length}/${missing.length} screenshots`);
            const successByUrl = new Map(successful.map((u) => [u.url, u]));
            // Phase 1: persist via the dual-write helper so both the embedded
            // array (legacy reader fallback) and the AuditPageResult collection
            // pick up the recovered Cloudinary URLs.
            const patches = new Map();
            for (const [url, u] of successByUrl) {
              patches.set(url, {
                screenshotDesktop: u.screenshotDesktop,
                screenshotMobile: u.screenshotMobile,
              });
            }
            await applyBulkUpdates({ auditId }, patches);
            // Mutate in-memory pageResults so the vision step + screenshotsData
            // computation below see the recovered URLs without another read.
            for (const pr of pageResults) {
              const u = successByUrl.get(pr.url);
              if (u) {
                pr.screenshotDesktop = u.screenshotDesktop;
                pr.screenshotMobile = u.screenshotMobile;
              }
            }
          }
        }
      } finally {
        if (retryScanner?.browser) {
          await retryScanner.browser.close().catch(() => {});
        }
      }
    }
    ...
    if (visionPages.length > 0) {
      ...
      try {
        ...
        logAuditError(auditId, 'vision-failed', err.message);
      }
    }
    ...
    try {
      ...
      logAuditError(auditId, 'summary-failed', err.message);
    }
    ...
    // Pre-compute site-wide flags so the dashboard never has to scan all
    // issues to derive them. Phase 4b: shrinks the audit response payload
    // (we no longer ship `issues` to the client) and keeps these UI-critical
    // booleans available without a round-trip.
    ...
    const siteWideIndicators = {
      hasSiteWideNoindex: trimmedIssues.some(
        (i) => i.message === 'audit.issues.wpSearchEngineDiscouraged' && i.severity !== 'passed'
      ),
      missingSecurityHeaders: Array.from(new Set(
        trimmedIssues
          .filter((i) => SECURITY_HEADER_KEYS.has(i.message) && i.severity !== 'passed')
          .map((i) => i.message)
      )),
    };

    // SiteAudit metadata write — issues handled separately via helper below
    // so the dual-write story is centralized.
    await dbWrite(() =>
      prisma.siteAudit.update({
        where: { id: auditId },
        data: {
    ...
    // Phase 2: replace the entire issues set via helper. This is the dedup +
    // trim final state; per-page persists during the chunk wrote interim
    // versions. Using replaceAllIssues guarantees both stores converge to
    // the trimmed/deduped final set rather than accumulating stale entries.
    await replaceAllIssues(
      { auditId, siteId: audit.siteId, accountId: site.accountId },
      trimmedIssues,
    ).catch((err) => {
      console.warn(`[SiteAudit/chunked] runFinalization replaceAllIssues failed: ${err.message}`);
    });
