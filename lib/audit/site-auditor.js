...
export async function runSiteAudit(auditId, siteUrl, siteId, deviceType, options = {}) {
  ...
  try {
    ...
    if (options.urls?.length) {
      ...
      method = 'explicit';
      hasSitemap = true; // Don't abort - user explicitly chose these URLs
    ...
    try {
      console.log('[SiteAudit] Generating AI summary...');
        summary = await generateAuditSummary(deduped, score, categoryScores, url, enrichedPageResults.length, { accountId: site.accountId, siteId, userId: options.userId });
      if (summary) {
