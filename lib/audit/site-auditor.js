...
export async function runSiteAudit(auditId, siteUrl, siteId, deviceType) {
  ...
  try {
    ...
    const site = await prisma.site.findUnique({
      where: { id: siteId },
      select: { id: true, url: true, name: true, accountId: true, connectionStatus: true, siteKey: true, siteSecret: true, platform: true },
    });
    ...
    try {
      ...
      summary = await generateAuditSummary(deduped, score, categoryScores, url, enrichedPageResults.length);
      if (summary) {
    ...
    await dbWrite(() =>
      prisma.siteAudit.update({
        ...
        ...(summary ? { summary, summaryTranslations: { en: summary } } : {}),
      },
    })
    );
...
async function scanSinglePage(pageUrl, scanner, isHomepage, runPsi, deviceType) {
  ...
  if (scanner) {
    try {
      ...
      if (html) {
        const pwHeaders = {};
        if (pwResult.xRobotsTag) pwHeaders['x-robots-tag'] = pwResult.xRobotsTag;
        const htmlIssues = analyzeHtml(html, pageUrl, pwHeaders, pwResult.ttfb, pwResult.dom);
