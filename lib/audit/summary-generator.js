 ...
 */
export async function generateAuditSummary(issues, score, categoryScores, siteUrl, pagesScanned, context = {}) {
...
Write a concise summary.`;
  ...
  try {
    ...
    if (context.accountId) {
      await deductAiCredits(context.accountId, 1, {
        userId: context.userId,
        siteId: context.siteId,
        source: 'audit_summary',
        description: `Audit summary for ${siteUrl}`,
        metadata: {
          model: GEMINI_MODEL,
...
// ISO-code → human name for the translator system prompt.
// Kept small intentionally — unknown codes fall back to the raw code and
// Gemini still produces a correct translation for any BCP-47 tag it knows.
const LANG_NAMES = {
  en: 'English',
  he: 'Hebrew',
  ar: 'Arabic',
...
/**
 * Translate an already-generated English audit summary to another language.
 * Deducts 1 AI credit on success. Returns trimmed text or null on failure.
 *
 * Used both from `/api/audit/translate-summary` (user-triggered) and from
 * the audit pipeline to pre-cache the initiating user's locale.
 *
 * @param {string} sourceSummary - English source summary
 * @param {string} targetLang - Target language code (e.g. 'he')
 * @param {{ accountId?: string, userId?: string, siteId?: string, siteUrl?: string }} context
 * @returns {Promise<string|null>}
 */
export async function translateAuditSummary(sourceSummary, targetLang, context = {}) {
  if (!sourceSummary || !targetLang || targetLang === 'en') return null;
...
Only output the translated text - no preamble or explanation.`,
    ...
    if (context.accountId) {
      ...
      await deductAiCredits(context.accountId, 1, {
        ...
        description: `Translate audit summary to ${langName}${context.siteUrl ? ` for ${context.siteUrl}` : ''}`,
        metadata: {
          model: GEMINI_MODEL,
          targetLang,
          inputTokens: usage.inputTokens || 0,
    ...
    console.error('[SummaryGenerator] Translation failed:', err.message);
    return null;
  }
}
