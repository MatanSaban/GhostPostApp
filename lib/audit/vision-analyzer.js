...
export async function analyzeVisualIssues(pages, siteUrl, context = {}) {
  ...
  for (const page of validPages) {
    ...
    // Each image can be a Buffer (legacy single-shot path: screenshots stay
    // in memory across the whole audit) or a string URL (new chunked path:
    // screenshots are uploaded to Cloudinary inline per-page, freed, and
    // re-fetched here from those URLs). Wrap strings in URL() so the AI SDK
    // unambiguously routes them through its fetcher.
    const asImage = (v) => (typeof v === 'string' ? new URL(v) : v);
    if (page.desktop) {
      content.push(
        { type: 'text', text: `DESKTOP Screenshot (1920×1080) - ${pathLabel}:` },
        { type: 'image', image: asImage(page.desktop) }
      );
    }
    if (page.mobile) {
      content.push(
        { type: 'text', text: `MOBILE Screenshot (375×812, iPhone) - ${pathLabel}:` },
        { type: 'image', image: asImage(page.mobile) }
      );
    }
  }
