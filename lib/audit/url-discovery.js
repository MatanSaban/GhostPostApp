 ...
 *
 * 2. GhostSEO Plugin DB entities (all synced content with URLs)
 *
 * 3. GhostSEO SiteSitemap records (previously scanned sitemaps in DB)
 *
...
import { filterToDefaultLanguage } from './locale-filter.js';
...
function dedupeWithHome(homeUrl, urls, maxUrls = DEFAULT_MAX_URLS) {
...
async function discoverFromSitemap(baseUrl) {
  ...
  async function processSitemap(sitemapUrl, depth = 0) {
    ...
    try {
      ...
      for (const childUrl of childSitemaps) {
        if (allUrls.size >= DEFAULT_MAX_URLS) break;
  ...
  return [...allUrls].slice(0, DEFAULT_MAX_URLS);
}

// ═══════════════════════════════════════════════════════════
// STRATEGY 2: GhostSEO Plugin - Live API + DB Entities
// ═══════════════════════════════════════════════════════════

/**
 * Fetch all post/page URLs from the connected WordPress site
 * via the GhostSEO plugin REST API (authenticated HMAC).
...
async function discoverFromPlugin(siteId) {
  ...
  if (site?.siteKey && site?.siteSecret) {
    ...
    try {
      ...
      const siteInfoRes = await fetch(`${baseUrl}/wp-json/ghostseo/v1/site-info`, {
        headers: { ...siteInfoHeaders, ...BOT_FETCH_HEADERS },
    ...
    for (const endpoint of endpoints) {
      try {
        const apiUrl = `${baseUrl}/wp-json/ghostseo/v1${endpoint}?per_page=100&full=false`;
        const headers = createHmacHeaders(site.siteKey, site.siteSecret, '');

        const res = await fetch(apiUrl, {
          headers: { ...headers, ...BOT_FETCH_HEADERS },
    ...
    if (urls.length > 0) {
      ...
      return [...new Set(urls)].slice(0, DEFAULT_MAX_URLS);
    }
  ...
  try {
    const entities = await prisma.siteEntity.findMany({
      where: { siteId, url: { not: null }, entityType: { isEnabled: true } },
      select: { url: true, status: true },
      orderBy: { publishedAt: 'desc' },
      take: DEFAULT_MAX_URLS * 2,
    });
...
// ═══════════════════════════════════════════════════════════
// STRATEGY 3: GhostSEO DB - Stored Sitemaps
// ═══════════════════════════════════════════════════════════
...
async function discoverFromStoredSitemaps(siteId, baseUrl) {
  try {
    ...
    for (const sm of storedSitemaps) {
      ...
      if (allUrls.length >= DEFAULT_MAX_URLS) break;
    }
    ...
    return [...new Set(allUrls)].slice(0, DEFAULT_MAX_URLS);
  } catch (err) {
...
async function discoverFromCrawl(baseUrl) {
  ...
  if (urls.size >= DEFAULT_MAX_URLS) return [...urls].slice(0, DEFAULT_MAX_URLS);
  ...
  const secondLevelFetches = secondLevelBatch.map(async (pageUrl) => {
    try {
      ...
      for (const u of newLinks) {
        if (urls.size >= DEFAULT_MAX_URLS) break;
        urls.add(u);
        visited.add(u);
      }
  ...
  return [...urls].slice(0, DEFAULT_MAX_URLS);
}
 ...
 *
 * @param {Object} site - Site record from DB ({ id, url, connectionStatus, contentLanguage? })
 * @param {Object} [options] - Discovery options
 * @param {number} [options.maxUrls] - Maximum URLs to discover (default: DEFAULT_MAX_URLS)
 * @returns {{ urls: string[], method: string, hasSitemap: boolean }}
 */
export async function discoverUrls(site, options = {}) {
  const maxUrls = options.maxUrls || DEFAULT_MAX_URLS;
  const homeUrl = normalizeUrl(site.url);
  const contentLanguage = site.contentLanguage || null;
  let hasSitemap = false;
  console.log(`[URLDiscovery] Starting URL discovery for ${homeUrl} (connection: ${site.connectionStatus}, maxUrls: ${maxUrls}, lang: ${contentLanguage || 'any'})`);

  function applyLocaleFilter(urls) {
    if (!contentLanguage) return urls;
    const before = urls.length;
    const filtered = filterToDefaultLanguage(urls, homeUrl, contentLanguage);
    if (filtered.length < before) {
      console.log(`[URLDiscovery] Locale filter: ${before} → ${filtered.length} URLs (removed ${before - filtered.length} alternate-language URLs)`);
    }
    return filtered;
  }
  ...
  if (sitemapUrls.length > 1) {
    ...
    return { urls: dedupeWithHome(homeUrl, applyLocaleFilter(sitemapUrls), maxUrls), method: 'sitemap', hasSitemap };
  }
  console.log(`[URLDiscovery] ✗ No sitemap found`);

  // ── 2. GhostSEO Plugin: Live API + DB entities ──
  if (site.connectionStatus === 'CONNECTED') {
    ...
    if (pluginUrls.length > 0) {
      ...
      return { urls: dedupeWithHome(homeUrl, applyLocaleFilter(pluginUrls), maxUrls), method: 'plugin', hasSitemap };
    }
    console.log(`[URLDiscovery] ✗ Plugin returned 0 URLs`);
  }

  // ── 3. Stored sitemaps from GhostSEO DB ──
  ...
  if (storedUrls.length > 0) {
    ...
    return { urls: dedupeWithHome(homeUrl, applyLocaleFilter(storedUrls), maxUrls), method: 'stored-sitemap', hasSitemap };
  }
  ...
  if (wpUrls.length > 0) {
    ...
    return { urls: dedupeWithHome(homeUrl, applyLocaleFilter(wpUrls), maxUrls), method: 'wp-api', hasSitemap };
  }
  ...
  return { urls: dedupeWithHome(homeUrl, applyLocaleFilter(crawledUrls), maxUrls), method: 'crawl', hasSitemap };
}
