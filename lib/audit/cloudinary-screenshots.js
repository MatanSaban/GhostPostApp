...
// ─── Module-level upload semaphore ───────────────────────────────────────────
//
// Each audited page can produce ~30 uploads (desktop + mobile screenshots,
// ~10 segmented chunks per device, ~5 filmstrip frames per device). With
// page-scan concurrency=3 in the chunked auditor, that's nearly 100 sockets
// held at once during a heavy chunk. Two real consequences seen in practice:
//
//   • Cloudinary "Request Timeout" failures cascade — too many in-flight
//     POSTs against a single client connection budget.
//   • The same Node process can't get sockets to MongoDB Atlas, so the
//     Prisma connection pool clears and unrelated queries (auth, watchdog,
//     fix endpoints) start failing with `os error 10060`.
//
// Cap concurrency at the source so every caller benefits: legacy path,
// chunked path, rescan, recheck. 6 is enough to keep throughput up while
// leaving the OS socket pool with headroom for Mongo + Playwright.
let _uploadLimit = null;
function uploadLimit() {
  if (_uploadLimit) return _uploadLimit;
  const max = parseInt(process.env.CLOUDINARY_MAX_CONCURRENT || '6', 10);
  let active = 0;
  const queue = [];
  const runNext = () => {
  ...
  _uploadLimit = (fn) => new Promise((resolve, reject) => {
 ...
 * Includes a 30s timeout and 1 retry on failure.
 *
 * Routed through a process-wide concurrency limiter so a chunk-full of
 * pages can't simultaneously open ~100 outbound HTTPS sockets and starve
 * MongoDB / Playwright of network resources.
 *
...
async function uploadBuffer(buffer, folder, publicId) {
  ensureConfig();
  return uploadLimit()(() => uploadBufferInner(buffer, folder, publicId));
}

async function uploadBufferInner(buffer, folder, publicId) {
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      if (attempt < MAX_RETRIES) {
        console.warn(`[Cloudinary] Upload failed (attempt ${attempt + 1}), retrying: ${err.message}`);
 ...
 *
 * Format: ghostseo/{accountSlug}/{siteHost}/{datetime}_{seq}
 *
 * Screenshots uploaded under any earlier folder prefix remain accessible because
 * their full secure_url was stored in the database at upload time - Cloudinary
 * serves whatever URL is requested and does not require the folder prefix to
...
function buildAuditFolder(accountSlug, siteUrl, auditDate, auditSeq) {
  ...
  return `ghostseo/${accountSlug}/${host}/${ts}_${auditSeq}`;
}
