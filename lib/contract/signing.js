...
// ── Previous keys (rotation) ────────────────────────────────────────────
// CONTRACT_SIGNING_PREVIOUS_KEYS holds retired signing keys (comma-separated
// base64 PKCS8 PEMs, newest first). They are VERIFY-ONLY: /contract-key
// publishes their public halves so consumers keep verifying envelopes signed
// before a rotation, but signPayload always signs with the current key.
let cachedPrevious = null;

function loadPreviousKeys() {
  if (cachedPrevious) return cachedPrevious;
  const raw = process.env.CONTRACT_SIGNING_PREVIOUS_KEYS || '';
  const keys = [];
  for (const part of raw.split(',').map((s) => s.trim()).filter(Boolean)) {
    try {
      const pem = part.includes('BEGIN') ? part : Buffer.from(part, 'base64').toString('utf8');
      ...
      console.warn('[contract/signing] Skipping unparsable previous key:', err?.message);
    }
  }
  cachedPrevious = keys;
  return keys;
 ...
 * `keys` lists every currently-trusted verification key (current first, then
 * previous) so consumers can verify across a rotation window.
 * @returns {{ alg: string, keyId: string, publicKeyPem: string, ephemeral: boolean, keys: Array<{keyId: string, publicKey: string}> }}
 */
export function getPublicKeyInfo() {
  const { keyId, publicKeyPem, ephemeral } = loadKey();
  const keys = [
    { keyId, publicKey: publicKeyPem },
    ...loadPreviousKeys().map((k) => ({ keyId: k.keyId, publicKey: k.publicKeyPem })),
  ];
  return { alg: 'ed25519', keyId, publicKeyPem, ephemeral, keys };
}
