...
export async function getAccountUsage(accountId, resourceKey) {
  ...
  if (resourceKey === 'aiCredits') {
    // Split add-on bonus: recurring (resets per period) vs one-time (persists until consumed)
    ...
    for (const p of addOnPurchases) {
      ...
      if (p.addOn.billingType === 'ONE_TIME') {
        oneTimePool += p.creditsRemaining || 0;
      } else {
        recurringBonus += (p.addOn.quantity || 1) * qty;
      }
    }
    ...
    const totalLimit = effectivePeriodAlloc + oneTimePool;
