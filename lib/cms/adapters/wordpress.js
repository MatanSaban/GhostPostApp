...
/**
 * Restore the rollback snapshot returned by manipulate_element. Accepts the
 * exact shape the manipulator emits under its `rollback` key:
 *   { meta_key: '_elementor_data'|'_fl_builder_data', previous_value: <json|array> }
 *   { post_content: <html> }
 * Plugin endpoint added in 5.0.1. Older plugins return 404 here; callers
 * should surface that as an upgrade prompt.
 *
 * @param {Object} site
 * @param {number|string} postId
 * @param {{ meta_key?: string, previous_value?: any, post_content?: string }} snapshot
 */
export async function restoreElementSnapshot(site, postId, snapshot) {
  return makePluginRequest(site, `/elements/restore/${parseInt(postId, 10)}`, 'POST', snapshot);
}
...
/**
 * Get a single media item by ID via the authenticated plugin endpoint.
 * Falls back to the public WP REST endpoint only when the plugin is too old
 * to expose /media/{id} GET (pre-1.3.0). Sites that block public /wp/v2 still
 * work as long as our plugin is current.
 * @param {Object} site - Site object with connection details
...
export async function getMediaItem(site, mediaId) {
  ...
  try {
    return await makePluginRequest(site, `/media/${cleanId}`, 'GET');
  } catch (err) {
    if (!/(404|rest_no_route)/i.test(err.message)) throw err;

    // Plugin is older than the version that introduced single-item GET. Fall
    // back to the public WP REST endpoint so existing flows keep working.
    const baseUrl = site.url.replace(/\/$/, '');
    const response = await fetch(`${baseUrl}/wp-json/wp/v2/media/${cleanId}`, {
      headers: BOT_FETCH_HEADERS,
      signal: AbortSignal.timeout(10000),
    });
    if (!response.ok) {
