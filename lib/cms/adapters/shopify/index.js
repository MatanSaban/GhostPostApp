...
function notImplemented(method) {
  const fn = () => {
    throw Object.assign(
      new Error(`[cms/shopify] ${method}() is not implemented yet.`),
      { code: 'NATIVE_WRITE_UNAVAILABLE', assisted: true },
    );
  };
  // Marker so the applyChange choke point degrades to the assisted path
...
function notSupported(method, reason) {
  const fn = () => {
    throw Object.assign(
      new Error(`[cms/shopify] ${method}() is not supported on Shopify. ${reason}`),
      { code: 'NATIVE_WRITE_UNAVAILABLE', assisted: true },
    );
  };
  fn.__notSupported = true;
...
export const selfUpdatePlugin = notSupported('selfUpdatePlugin', 'Shopify uses OAuth, not the GhostSEO Connector plugin.');
