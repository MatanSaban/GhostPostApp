...
export async function uploadMediaFromUrl(site, { url, filename, alt, contentType }) {
  const input = {
    alt: alt || null,
    contentType: (contentType || '').toUpperCase().includes('IMAGE') || /\.(png|jpe?g|webp|gif|svg)$/i.test(url)
      ? 'IMAGE'
      : 'FILE',
    originalSource: url,
  };
...
/** Upload from a base64 data URL (decoded client-side before here). */
export async function uploadMediaFromBase64(site, { base64, filename, mimeType, alt }) {
  const buffer = Buffer.from(base64.replace(/^data:[^;]+;base64,/, ''), 'base64');
  return uploadMediaFromBuffer(site, { buffer, filename, mimeType, alt });
}
