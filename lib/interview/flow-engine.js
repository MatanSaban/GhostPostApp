...
import { completeInterview as completeInterviewHandler } from '@/lib/bot-actions/handlers/complete-interview';
...
export async function getNextQuestion(interviewId) {
  ...
  // Find next valid question that hasn't been answered yet
...
export async function executeAutoActions(question, context) {
  ...
  for (const autoAction of question.autoActions) {
    ...
    try {
      ...
      // Get websiteUrl from responses for actions that need it
      const websiteUrl = context.responses?.websiteUrl || context.interview?.externalData?.crawledUrl;
      
      // Build params from the current question's response
      // The handler can use params.url or context.responses.websiteUrl
      // For URL-based actions that always need the website URL (not the question answer),
      // use websiteUrl directly to avoid passing non-URL responses (e.g., 'wordpress')
      const urlBasedActions = ['DETECT_PLATFORM', 'ANALYZE_WRITING_STYLE', 'FETCH_ARTICLES'];
      const params = {
        url: urlBasedActions.includes(actionName) ? websiteUrl : (questionResponse || websiteUrl),
        websiteUrl: websiteUrl, // Always include websiteUrl for actions like DETECT_PLATFORM
        response: questionResponse,
        // Pass the user's UI locale through to handlers that produce
        // human-readable text (CRAWL_WEBSITE / ANALYZE_WRITING_STYLE / etc.).
        // Handlers ignore unknown params so this is safe across the board.
...
export async function completeInterview(interviewId, context) {
  ...
  // Execute complete interview handler directly (not via executeAction which requires a BotAction DB record)
  try {
    console.log('[FlowEngine] Running completeInterview handler directly for interview:', interviewId);
    const result = await completeInterviewHandler(
      { interviewId, createAccount: true },
      {
        ...context,
        interview: context.interview || interview,
        interviewId,
        prisma,
      }
    );
    console.log('[FlowEngine] completeInterview handler result:', result?.success, result?.siteId ? `siteId: ${result.siteId}` : 'no siteId');
    return { ...interview, completionResult: result };
  } catch (error) {
    console.error('Error executing complete interview handler:', error);
  }
