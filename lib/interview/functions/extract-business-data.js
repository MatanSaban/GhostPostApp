 ...
 */
/**
 * @param {object} pagesData - { homeHtml, aboutHtml, contactHtml }
 * @param {string} url
 * @param {object} basicPageData
 * @param {object} [options]
 * @param {string} [options.userLocale] - User's UI locale ("he", "en", ...).
 *   When provided, AI text fields (description, category, services,
 *   targetAudience) are written in this locale rather than the detected
 *   site language. This keeps the interview chat in a single language even
 *   when the site itself is in another. Keywords/SEO terms still use the
 *   site language elsewhere in the pipeline.
 */
export async function extractBusinessDataWithAI(pagesData, url, basicPageData, options = {}) {
  ...
  // Detect the primary language from the website content (used for the
  // detectedLanguage echo field). The OUTPUT language is separate: when the
  // caller passed a userLocale we prefer that so the chat that renders these
  // fields stays in a single language even if the website itself is in
  // another. Falling back to detected language preserves old behavior when
  // no locale was supplied (e.g. server-side cron jobs).
  const detectedLang = detectPrimaryLanguage(combinedText);
  const outputLang = (userLocale || detectedLang || 'en').toLowerCase();
  const languageInstruction = getLanguageInstruction(outputLang);
  ...
  const schema = z.object({
    ...
    detectedLanguage: z.string().describe('The primary language code of the website itself (e.g., "he" for Hebrew, "en" for English, "es" for Spanish, etc.). This reflects the SITE language, not the output language.'),
...
Detected Site Language: ${detectedLang}
Output Language (REQUIRED for description/category/services/targetAudience): ${languageInstruction}

${combinedText}

CRITICAL INSTRUCTIONS:

**LANGUAGE**: All human-readable output fields (description, category, servicesOrProducts, targetAudience) MUST be written in ${languageInstruction}. This is the user's chat/UI language. Do NOT default to the site's language — translate the content into ${languageInstruction} regardless of what language the website is in. The detectedLanguage field below is the only place to report the site's own language.

1. **businessName**: Extract ONLY the actual company/business name (keep it as it appears on the site — brand names are not translated).
...
2. **description**: Write a proper "About Us" paragraph based on the content.
   ...
   - **WRITE IN ${languageInstruction.toUpperCase()} — translate the source content if needed.** Never mix two languages in this field.

3. **detectedLanguage**: Return the language code of the WEBSITE itself (e.g., "he", "en", "es", "fr", etc.). This is metadata, not a directive for the other fields.
...
4. **category**: Choose ONE specific main category that best describes the business
   ...
   - **WRITE IN ${languageInstruction.toUpperCase()}**

5. **address**: Look for street addresses, city names, zip codes
   - Check the contact page and footer areas
   - Format as a complete address string (keep proper nouns as written)
...
8. **servicesOrProducts**: List main offerings (max 5) — **IN ${languageInstruction.toUpperCase()}**
