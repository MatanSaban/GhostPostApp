...
export async function crawlWebsite(userInput, options = {}) {
  // userLocale is propagated to the AI extraction step so description /
  // category / services / targetAudience come back in the user's UI
  // language rather than the site's content language.
  const { skipSitemap = false, skipSEO = false, maxSitemapUrls = 50, userLocale } = options;
  ...
  try {
    ...
    const aiBusinessData = await extractBusinessDataWithAI(pagesData, result.url, result.pageData, { userLocale });
