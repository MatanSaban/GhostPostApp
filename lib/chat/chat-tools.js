...
export function getChatTools({ isWordPress = false }) {
  ...
  if (isWordPress) {
    ...
    tools.wp_self_update_plugin = {
      description: 'Force the Ghost Post plugin on this site to update itself to the latest published version. Use when the user\'s plugin is older than the version that introduced a tool/endpoint they need, or when they explicitly ask to update the plugin. Returns the new version or indicates already-latest.',
      inputSchema: jsonSchema({ type: 'object', properties: {} }),
    };
