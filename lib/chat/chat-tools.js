...
export const APPROVAL_REQUIRED_TOOLS = new Set([
  ...
  // Platform-neutral writes (custom / non-CMS sites) - routed through applyChange.
  'create_post',
  'update_seo',
  'create_redirect',
  'run_site_audit',
...
export function getChatTools({ isWordPress = false, isShopify = false }) {
  const tools = {};
  ...
  // ── Platform-neutral write tools (custom / non-CMS sites) ────────────
  // Exposed only when there is no native CMS tool set (not WordPress, not
  // Shopify). They route through the applyChange choke point: a native write
  // when a write transport is connected, otherwise a structured, copy-ready
  // result the user applies manually (ready-to-paste <head> tags, redirect
  // config, or post HTML). WordPress keeps its richer wp_* tools.
  if (!isWordPress && !isShopify) {
    tools.create_post = {
      description: 'Create a new page or blog post on the site. If the site has no connected write transport (SDK / edge proxy), this returns the ready-to-publish HTML + metadata for the user to add to their site - it does NOT modify the live site directly. Use it to draft a new article/page for the user.',
      inputSchema: jsonSchema({
        type: 'object',
        properties: {
          title: { type: 'string', description: 'Post/page title' },
          content: { type: 'string', description: 'Full HTML body content' },
          excerpt: { type: 'string', description: 'Short excerpt / summary' },
          slug: { type: 'string', description: 'URL slug (e.g. my-new-post)' },
    ...
    tools.update_seo = {
      description: 'Set or update the SEO meta for a page (title, description, canonical). If the site has no connected write transport, this returns the exact <head> tags to paste into that page - it does NOT modify the live site directly. Provide the page URL or path.',
      inputSchema: jsonSchema({
        type: 'object',
        properties: {
          url: { type: 'string', description: 'The page URL or path (e.g. /pricing)' },
    ...
    tools.create_redirect = {
      description: 'Create a redirect from one path to another. If the site has no connected write transport, this returns ready-to-paste redirect config for common hosts (Next.js, Netlify, Vercel, nginx, Apache) - it does NOT modify the live site directly.',
      inputSchema: jsonSchema({
        type: 'object',
        properties: {
          from: { type: 'string', description: 'Source path to redirect FROM (e.g. /old-page)' },
          to: { type: 'string', description: 'Destination path or URL to redirect TO' },
          type: { type: 'string', enum: ['301', '302'], description: 'Redirect type. Default 301 (permanent).' },
        },
        required: ['from', 'to'],
      }),
    };
