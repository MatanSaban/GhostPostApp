...
async function rollbackSingleAction(rb, site) {
  ...
  switch (tool) {
    ...
    case 'manipulate_element': {
      // The plugin's manipulator returns its rollback payload under the
      // `rollback` key of the response. We persisted that payload directly as
      // originalValue in the executor. Two shapes:
      //   { post_id, meta_key: '_elementor_data'|'_fl_builder_data', previous_value }
      //   { post_content: <html> }   (raw HTML / Gutenberg post)
      // We hand it straight back to /elements/restore/{id} and the plugin
      // writes the snapshot atomically + fires Elementor's after_save +
      // flushes per-post cache.
      const restorePostId = originalValue.post_id || args.postId;
      if (!restorePostId) {
        throw new Error('manipulate_element rollback is missing the target post_id');
      }
      try {
        await wpApi.restoreElementSnapshot(site, restorePostId, originalValue);
      } catch (err) {
        // Surface an actionable message when the plugin is older than 5.0.1
        // (route doesn't exist yet) so the user knows what to do.
        if (/(404|rest_no_route)/i.test(err.message)) {
          throw new Error(
            `Cannot rollback this Elementor edit - the WordPress plugin is older than 5.0.1 and does not support /elements/restore. Update the GhostSEO plugin on the site, then retry. Original error: ${err.message}`,
          );
        }
        throw err;
      }
    ...
    case 'scan_competitor_page':
      // These are read-only triggers / non-destructive lookups - nothing to undo.
      break;

    case 'add_competitor': {
      // Delete the competitor record we just inserted.
      if (originalValue?.competitorId) {
        try {
          const comp = await prisma.competitor.findUnique({
            where: { id: originalValue.competitorId },
            select: { siteId: true },
          });
          await prisma.competitor.delete({ where: { id: originalValue.competitorId } });
          if (comp?.siteId) invalidateCompetitors(comp.siteId);
        } catch (err) {
          // If the row is already gone (e.g. user deleted it manually), treat
          // rollback as a no-op rather than failing the whole rollback batch.
          if (!/Record to delete does not exist/i.test(err.message) && err.code !== 'P2025') {
            throw err;
          }
        }
      }
    ...
    case 'create_content_campaign': {
      // Delete the DRAFT campaign we created. We deliberately don't try to
      // touch a campaign the user already activated - status guard below.
      if (originalValue?.campaignId) {
        try {
          const camp = await prisma.campaign.findUnique({
            where: { id: originalValue.campaignId },
            select: { status: true },
          });
          if (camp && camp.status === 'DRAFT') {
            await prisma.campaign.delete({ where: { id: originalValue.campaignId } });
          }
        } catch (err) {
          if (!/Record to delete does not exist/i.test(err.message) && err.code !== 'P2025') {
            throw err;
    ...
    case 'create_backlink_listing': {
      // Soft-delete: mark the listing inactive instead of hard-deleting so
      // we keep audit history. If buyers already purchased, downstream
      // foreign keys protect against deletion anyway.
      if (originalValue?.listingId) {
        try {
          await prisma.backlinkListing.update({
            where: { id: originalValue.listingId },
            data: { isActive: false, status: 'ARCHIVED' },
          });
        } catch (err) {
          if (!/Record to update not found/i.test(err.message) && err.code !== 'P2025') {
            throw err;
          }
        }
      }
