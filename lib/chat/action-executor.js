...
export async function executeReadOnlyTool(toolName, args, { site, siteId, accountId }) {
  switch (toolName) {
    ...
    case 'get_site_audit_results': {
      ...
      // NB: SiteAudit.issues is an embedded composite (AuditIssue[] in the
      // MongoDB connector), NOT a relation - Prisma rejects `take` inside a
      // composite select with "Unknown argument `take`". Pull the full array
      // and trim in JS instead. (The audit writer already caps each issue's
      // payload defensively, so the array isn't unbounded.)
      const audit = await prisma.siteAudit.findFirst({
        where: { siteId, deviceType: device, status: 'COMPLETED' },
        orderBy: { createdAt: 'desc' },
        select: {
          id: true, score: true, categoryScores: true, deviceType: true,
          createdAt: true, summary: true, issues: true,
        },
      });
...
export async function executeChatAction(chatActionId) {
  ...
  try {
    ...
    const summary = results.map((r, i) => {
      ...
      const successLabel = isLongRunning ? t.started : t.done;
    ...
    await prisma.chatMessage.create({
      data: {
        conversationId: chatAction.conversationId,
        role: 'ASSISTANT',
        content: `### ${t.actionCompleted}\n\n${summary}${longRunningNotice}${verificationNote}${previewExtra}\n\n> ${t.rollbackHint}`,
      },
    });
