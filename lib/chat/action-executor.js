...
import { applyChange } from '@/lib/cms/apply';
...
import { refundChatActionCredits } from '@/lib/ai/credits-service';
import { webSearch } from '@/lib/web-search';
import { fetchUrl } from '@/lib/url-fetch';
...
export async function executeReadOnlyTool(toolName, args, { site, siteId, accountId }) {
  switch (toolName) {
    ...
    case 'get_site_audit_results': {
      ...
      // Phase 4b: issues live in the AuditIssue collection. Pull the audit
      // shell from SiteAudit, then sourcing issues through the helper so
      // the rollout flag (off/dual/on) is respected.
      ...
      const { getAllIssues } = await import('@/lib/audit/issues-helper.js');
      const allIssues = await getAllIssues(audit.id);
      // Surface errors and warnings first, then everything else - more useful
      // for the AI summarizer than a raw slice that may all be "passed" rows.
      const sortedIssues = [...allIssues].sort((a, b) => {
        const rank = (s) => (s === 'error' ? 0 : s === 'warning' ? 1 : s === 'info' ? 2 : 3);
        return rank(a.severity) - rank(b.severity);
      });
      return {
        ...audit,
        issues: sortedIssues.slice(0, 20),
    ...
    case 'web_search': {
      ...
      return await webSearch(args.query, { limit: args.limit || 5 });
    }
 ...
 */
export async function executeWriteAction(action, { site, siteId, accountId, userId, conversationId, chatActionId }) {
  ...
  switch (tool) {
    ...
    case 'generate_image': {
      ...
      if (!rawPrompt) throw new Error('generate_image requires a prompt');

      // Force any visible/rendered text inside the image to be in the SITE's
      // language - never the model's default English. Hebrew sites must
      // produce Hebrew text in hero images, etc. The rest of the prompt stays
      // in English (Nano Banana renders best from English subject/style cues),
      // we only constrain the language of any in-image rendered text.
      const promptText = applyImageLanguageDirective(rawPrompt, site);
      ...
      const images = await generateImage({
        ...
        userId,
        siteId,
        metadata: { source: 'chat', siteId, referenceCount: refImages.length, chatActionId },
      });
      ...
      const safeTitle = (args.title || rawPrompt).slice(0, 60).replace(/[^\w\u0590-\u05FF\s-]/g, '').trim() || 'generated-image';
      ...
      const uploaded = await wpApi.uploadMediaFromBase64(site, img.base64, filename, {
        title: args.title || rawPrompt.slice(0, 80),
        alt: args.alt || rawPrompt.slice(0, 120),
        caption: '',
        // Don't attach the upload to the post yet - that creates a visual
        // association before the user has confirmed the image. Attach only
        // when the follow-up wp_set_featured_image proposal is approved.
        postId: undefined,
      });

      const mediaId = uploaded?.id || uploaded?.data?.id || uploaded?.mediaId;
      const url = uploaded?.source_url || uploaded?.url || uploaded?.guid?.rendered || uploaded?.guid;

      // Preview-then-confirm flow: NEVER set the featured image inline. If
      // the user asked for it (setAsFeaturedFor), we generate + upload,
      // surface the image as a preview message, and let executeChatAction
      // create a SECOND pending ChatAction proposing wp_set_featured_image so
      // they can approve / reject / regenerate after seeing the result.
      // Auto-assigning silently is exactly the surprise the user pushed back on.
      let pendingFeatured = null;
      if (mediaId && args.setAsFeaturedFor) {
        pendingFeatured = {
          postId: args.setAsFeaturedFor,
          postType: args.setAsFeaturedPostType || 'posts',
          mediaId,
          imageUrl: url,
          alt: args.alt || rawPrompt.slice(0, 120),
      ...
      result = {
        mediaId,
        url,
        title: uploaded?.title?.rendered || uploaded?.title || args.title || rawPrompt.slice(0, 80),
        alt: args.alt || rawPrompt.slice(0, 120),
        mimeType: img.mimeType,
        aspectRatio,
        // Surfaced for executeChatAction -> creates the follow-up approval card.
        pendingFeatured,
        // Preserve previous shape for callers that read featuredAssignment.
      ...
      originalValue = {
        ...
        uploaded: true,
        mediaId,
        // Track only the upload for rollback; featured assignment is now its
        // own ChatAction with its own rollback path.
        featuredAssignment: null,
      };
      break;
    }
    ...
    case 'wp_insert_image_in_content': {
      ...
      if (!imgUrl) {
        throw new Error(
          'wp_insert_image_in_content requires either mediaId or imageUrl. '
          + 'When chaining after a generate_image / wp_upload_media step in the same plan, the executor auto-fills these from the prior step - '
          + 'you can also pass mediaId="{{prev.mediaId}}" / imageUrl="{{prev.url}}" explicitly to force the wiring.'
        );
      }
    ...
    case 'run_site_audit': {
      // Create audit records. SiteAudit has no `url` column - the URL lives on
      // the Site row via siteId. Passing it here used to throw
      // "Unknown argument `url`" and crash the whole approve flow.
      const desktopAudit = await prisma.siteAudit.create({
        data: { siteId, deviceType: 'desktop', status: 'PENDING' },
      });
      const mobileAudit = await prisma.siteAudit.create({
        data: { siteId, deviceType: 'mobile', status: 'PENDING' },
      });
      // Pass the chat conversation id through so when the audit eventually
      // completes it can post a follow-up message back into THIS conversation
      // ("audit finished, score X/100, see report" or "audit failed: …").
      // Without this the user only ever sees the kickoff message and is left
      // wondering whether the audit ever finished.
      // ONLY desktop carries the chat conversationId so the user gets ONE
      // follow-up message even though we kick off two audits.
      // Fire and forget
      runSiteAudit(desktopAudit.id, site.url, siteId, 'desktop', {
      ...
      runSiteAudit(mobileAudit.id, site.url, siteId, 'mobile', {
        maxPages: args.maxPages,
        userId,
      }).catch(e =>
    ...
    case 'run_agent_scan': {
      ...
      // Same pattern as run_site_audit - pass the conversation id so the
      // analyzer can post a "scan finished, X new insights" follow-up here.
      runSiteAnalysis(siteId, accountId, 'manual', run.id, userId, { chatConversationId: conversationId }).catch(e =>
    ...
    // ── Platform-neutral writes (custom / non-CMS sites) ──────────────
    // Route through the applyChange choke point: a native write when a
    // transport is connected, otherwise a structured, copy-ready result the
    // user applies manually (the same tools go native automatically once an
    // SDK / edge-proxy transport is connected).
    case 'create_post': {
      const data = {
        title: args.title,
        content: args.content || '',
      ...
      if (args.metaTitle || args.metaDescription) {
        data.meta = {
          _yoast_wpseo_title: args.metaTitle || '',
          _yoast_wpseo_metadesc: args.metaDescription || '',
        };
      }
      const outcome = await applyChange(site, 'createPost', ['post', data], { manualKinds: ['snippet'] });
      if (outcome.mode === 'error') throw new Error(outcome.error || 'create_post failed');
      if (outcome.applied) {
        result = { native: true, post: outcome.result, postId: outcome.result?.id };
        originalValue = { created: true, postId: outcome.result?.id };
      } else {
        result = { assisted: true, summary: `Draft: "${args.title}"`, manual: buildAssistedPostOutput(args) };
        originalValue = { assisted: true };
      }
      break;
    }
    ...
    case 'update_seo': {
      ...
      const outcome = await applyChange(site, 'updateSeoData', [args.url, seoData], { manualKinds: ['snippet'] });
      if (outcome.mode === 'error') throw new Error(outcome.error || 'update_seo failed');
      if (outcome.applied) {
        result = outcome.mode === 'contract'
          ? {
              contract: true,
              result: outcome.result,
              message: `SEO for ${args.url} applied via the GhostSEO contract — live on the site after its next cache refresh (up to ~10 minutes).`,
            }
          : { native: true, result: outcome.result };
        originalValue = { seoUpdated: true, url: args.url };
      } else {
        // No native transport: persist the desired SEO to the Contract store so
        // it applies automatically once an SDK/edge connects, and hand the user
        // the ready-to-paste tags for now.
        const persisted = await upsertSeoOverride(site.id, args.url, {
          title: args.title,
          description: args.description,
          canonical: args.canonical,
        });
        result = { assisted: true, persisted: persisted.persisted, summary: `SEO tags for ${args.url}`, manual: buildAssistedSeoOutput(args) };
        originalValue = { assisted: true };
      }
      break;
    }
    ...
    case 'create_redirect': {
      const redirectData = {
        source: args.from,
        target: args.to,
        type: args.type === '302' ? 'TEMPORARY' : 'PERMANENT',
      };
      const outcome = await applyChange(site, 'createRedirect', [redirectData], { manualKinds: ['redirect'] });
      if (outcome.mode === 'error') throw new Error(outcome.error || 'create_redirect failed');
      if (outcome.applied) {
        result = outcome.mode === 'contract'
          ? {
              contract: true,
              result: outcome.result,
              message: `Redirect ${args.from} → ${args.to} applied via the GhostSEO contract — live after the site's next cache refresh (up to ~10 minutes; next.config redirects need a redeploy).`,
            }
          : { native: true, result: outcome.result };
        originalValue = { redirectCreated: true, from: args.from, to: args.to };
      } else {
        // No native transport: persist the redirect to the platform's table so
        // the Contract serves it (SDK/edge apply it), plus hand over host config.
        const persisted = await upsertRedirectOverride(site.id, args.from, args.to, args.type);
        result = { assisted: true, persisted: persisted.persisted, summary: `Redirect ${args.from} → ${args.to}`, manual: buildAssistedRedirectOutput(args) };
        originalValue = { assisted: true };
      }
      break;
    }
...
// ─── Assisted output builders (sites with no native write transport) ───
// Produce ready-to-paste artifacts so the user can apply the change manually.
...
function buildAssistedSeoOutput(args) {
  const tags = [];
  if (args.title) {
    tags.push(`<title>${escapeHtmlAttr(args.title)}</title>`);
    tags.push(`<meta property="og:title" content="${escapeHtmlAttr(args.title)}">`);
  }
  if (args.description) {
    tags.push(`<meta name="description" content="${escapeHtmlAttr(args.description)}">`);
    tags.push(`<meta property="og:description" content="${escapeHtmlAttr(args.description)}">`);
  }
  if (args.canonical) tags.push(`<link rel="canonical" href="${escapeHtmlAttr(args.canonical)}">`);
  return { kind: 'snippet', language: 'html', where: `In the <head> of ${args.url}`, code: tags.join('\n') };
}

function buildAssistedRedirectOutput(args) {
  const code = args.type === '302' ? 302 : 301;
  ...
  return {
    ...
    statusCode: code,
    configs: {
      nextConfig: `// next.config.js\nasync redirects() {\n  return [{ source: '${from}', destination: '${to}', permanent: ${code === 301} }];\n}`,
      netlify: `${from}  ${to}  ${code}`,
      nginx: `rewrite ^${from}$ ${to} redirect;`,
      apache: `Redirect ${code} ${from} ${to}`,
    },
  };
}

function buildAssistedPostOutput(args) {
  return {
    kind: 'post',
    title: args.title,
    slug: args.slug || '',
...
// Render an assisted manual output as markdown for the chat completion message.
function renderAssistedManual(m) {
  if (!m) return '';
  if (m.kind === 'snippet') {
    return `**${m.where || 'Add this'}:**\n\`\`\`${m.language || 'html'}\n${m.code}\n\`\`\``;
  }
  if (m.kind === 'redirect') {
    const c = m.configs || {};
    return `**Redirect ${m.from} → ${m.to} (${m.statusCode})** — paste into your host config:\n`
      + `\n_Next.js (next.config.js)_\n\`\`\`js\n${c.nextConfig}\n\`\`\``
      + `\n_Netlify (_redirects)_\n\`\`\`\n${c.netlify}\n\`\`\``
      + `\n_nginx_\n\`\`\`\n${c.nginx}\n\`\`\``
      + `\n_Apache (.htaccess)_\n\`\`\`\n${c.apache}\n\`\`\``;
  }
  if (m.kind === 'post') {
    return `**${m.title}**${m.slug ? ` (slug: \`${m.slug}\`)` : ''}`
      + (m.metaTitle ? `\n_Meta title:_ ${m.metaTitle}` : '')
      + (m.metaDescription ? `\n_Meta description:_ ${m.metaDescription}` : '')
      + `\n\n\`\`\`html\n${m.html}\n\`\`\``;
  }
  return '';
}
...
const ACTION_STRINGS = {
  EN: {
    ...
    assistedHint: 'This site has no connected write transport, so I prepared the change(s) for you - copy them into your site to apply.',
    savedNote: '✅ Saved to your GhostSEO SEO settings - it applies automatically once you connect the SDK or proxy.',
    failed: '❌ Failed',
    error: 'Error',
    rollbackHint: 'You can rollback these changes if needed.',
    rollbackPartialHint: 'You can rollback the completed steps.',
    longRunningNotice: '⏳ **This will take a few minutes.** You can keep working in the platform - I\'ll send you a follow-up message right here when it\'s done. If the chat is closed, you\'ll see an unread indicator on the chat bubble.',
    ...
    auditReady: '✅ **Site audit finished.** Score: {score}/100 across {pages} page(s). Open the [audit report]({link}) to see full details.',
    auditFailed: '❌ **Site audit failed:** {error}. You can retry from the [audit page]({link}) or ask me to run it again.',
    agentScanReady: '✅ **AI agent scan finished.** Generated {count} new insight(s). Open the [agent dashboard]({link}) to review them.',
    agentScanFailed: '❌ **AI agent scan failed:** {error}. Open the [agent dashboard]({link}) or ask me to retry.',
  },
  HE: {
    ...
    started: '🚀 התחיל',
    readyToApply: '📋 מוכן ליישום',
    assistedHint: 'לאתר הזה אין ערוץ כתיבה מחובר, אז הכנתי עבורך את השינויים - העתק אותם לאתר שלך כדי להחיל אותם.',
    savedNote: '✅ נשמר בהגדרות ה-SEO של GhostSEO - יוחל אוטומטית ברגע שתחבר את ה-SDK או ה-proxy.',
    failed: '❌ נכשל',
    error: 'שגיאה',
    rollbackHint: 'ניתן לבטל את השינויים במידת הצורך.',
    rollbackPartialHint: 'ניתן לבטל את הצעדים שהושלמו.',
    longRunningNotice: '⏳ **הפעולה הזו לוקחת כמה דקות.** תוכל להמשיך לעבוד בפלטפורמה - אני אשלח לך הודעת המשך כאן בצ׳אט ברגע שהיא תסתיים. אם הצ׳אט סגור, תראה סימון של הודעה ממתינה על כפתור הצ׳אט.',
    ...
    auditReady: '✅ **סריקת האתר הסתיימה.** ציון: {score}/100 על פני {pages} דפים. לחץ על [דו"ח הסריקה]({link}) כדי לראות את הפרטים המלאים.',
    auditFailed: '❌ **סריקת האתר נכשלה:** {error}. תוכל לנסות שוב מ[עמוד הסריקה]({link}) או לבקש ממני להריץ שוב.',
    agentScanReady: '✅ **סריקת ה-AI Agent הסתיימה.** נוצרו {count} תובנות חדשות. לחץ על [לוח ה-Agent]({link}) כדי לעיין בהן.',
    agentScanFailed: '❌ **סריקת ה-AI Agent נכשלה:** {error}. לחץ על [לוח ה-Agent]({link}) או בקש ממני להריץ שוב.',
  },
};

/**
 * Tools whose execute() returns immediately ("started") but whose actual work
 * runs in the background and posts a follow-up message to the chat when it
 * eventually finishes. Used to swap the "✅ Done" line for a "🚀 Started"
 * line + long-running notice in the action-completion summary.
 */
...
export async function executeChatAction(chatActionId) {
  ...
  const context = {
    ...
    // Stamped on every credit-tracked operation that runs inside this action
    // (image generation, AI locator, etc.) so refundChatActionCredits can
    // sum them up if the action ultimately fails.
    chatActionId: chatAction.id,
  };
  ...
  try {
    for (let stepIdx = 0; stepIdx < chatAction.actions.length; stepIdx++) {
      const action = chatAction.actions[stepIdx];
      // Resolve cross-step dependencies BEFORE executing this step. The AI
      // commonly chains generate_image → wp_insert_image_in_content /
      // wp_set_featured_image but proposes the second step without a
      // mediaId/imageUrl, expecting the system to wire them together. Without
      // this resolver the second step crashes with "requires either mediaId
      // or imageUrl" (the original failure on the dgblog conversation).
      // Two strategies, in order:
      //   (a) explicit template substitution in args - {{steps[N].mediaId}},
      //       {{prev.mediaId}}, {{prev.url}} - resolved against prior results.
      //   (b) heuristic auto-fill for known image-consumer tools: if the AI
      //       omitted mediaId/imageUrl on a tool that needs one, look back
      //       for the most recent successful generate_image / wp_upload_media
      //       in this same plan and pull mediaId + url from there.
      const resolvedAction = resolveCrossStepArgs(action, results);
      const outcome = await executeWriteAction(resolvedAction, context);
      results.push({
        tool: action.tool,
    ...
    // For long-running tools (run_site_audit / run_agent_scan) the execute()
    // call only KICKS OFF the background job - the work itself takes minutes
    // and posts its own follow-up message via on-complete hooks below. Show
    // "🚀 Started" instead of "✅ Done" so the user doesn't think the audit
    // is finished when only the kickoff succeeded.
    const summary = results.map((r, i) => {
      const action = chatAction.actions[i];
      const isLongRunning = action && LONG_RUNNING_TOOLS.has(action.tool);
      const assisted = !!r.result?.assisted;
      const successLabel = assisted ? t.readyToApply : (isLongRunning ? t.started : t.done);
      return `${i + 1}. **${r.description}** - ${r.success ? successLabel : t.failed}`;
    }).join('\n');

    // Assisted (no-native-transport) writes: append the ready-to-paste output
    // so the user can apply it. Only shown for steps that came back assisted.
    const assistedBlocks = results
      .filter((r) => r.result?.assisted && r.result?.manual)
      .map((r) => {
        const body = renderAssistedManual(r.result.manual);
        if (!body) return '';
        // When we persisted the change to the Contract store, tell the user it
        // will auto-apply once a transport connects (not just a throwaway snippet).
        return r.result.persisted ? `${t.savedNote}\n\n${body}` : body;
      })
      .filter(Boolean);
    const assistedExtra = assistedBlocks.length
      ? `\n\n${t.assistedHint}\n\n${assistedBlocks.join('\n\n')}`
      : '';

    // Only offer rollback when at least one step actually wrote to the site.
    const anyNativeWrite = results.some((r) => r.success && r.result && !r.result.assisted);
    const rollbackTail = anyNativeWrite ? `\n\n> ${t.rollbackHint}` : '';

    // Build a notice block for any long-running step that just kicked off
    // (not a per-step line - one notice covers the batch). It tells the user
    // they can keep working and that the chat bubble will signal the result.
    const longRunningCount = chatAction.actions.filter((a) => LONG_RUNNING_TOOLS.has(a.tool)).length;
    const longRunningNotice = longRunningCount > 0 ? `\n\n${t.longRunningNotice}` : '';
    ...
    // Image preview-then-confirm flow: if any generate_image step left a
    // pendingFeatured marker, surface the image as a preview + create a NEW
    // PENDING_APPROVAL ChatAction proposing wp_set_featured_image. The user
    // will see two cards in sequence: (1) "Image generated" success, then
    // (2) "Use this image as featured?" with the image embedded. This is what
    // replaces the old auto-assign-on-generate behavior.
    let previewExtra = '';
    try {
      for (let i = 0; i < results.length; i++) {
        const r = results[i]?.result;
        const pf = r?.pendingFeatured;
        if (!pf || !pf.mediaId || !pf.postId) continue;

        const isHe = lang === 'HE';
        const previewTitle = isHe ? '🖼️ תמונה נוצרה - אישור לפני שיוך' : '🖼️ Image generated - confirm before assigning';
        const previewDesc = isHe
          ? `**הנה התמונה שיצרתי לפי הבריף שלך:**\n\n![${pf.alt || ''}](${pf.imageUrl})\n\nאם זה נראה מצוין - אשר את הכרטיס למטה ואני אגדיר אותה כתמונה הראשית של הפוסט.\n\nאם תרצה שינוי - דחה את הכרטיס וכתוב לי איך לשפר את הפרומפט (למשל "סגנון אחר", "צבעים אחרים", "בלי טקסט", "לתמונה לוגו"), ואני אצור אחת חדשה.\n\n**פרומפט מקורי:**\n> ${pf.originalPrompt}`
          : `**Here's the image I generated for you:**\n\n![${pf.alt || ''}](${pf.imageUrl})\n\nIf it looks great, approve the card below and I'll set it as the featured image.\n\nIf you'd like changes, reject the card and tell me what to tweak in the prompt (e.g. "different style", "different colors", "no text", "include a logo") and I'll regenerate.\n\n**Original prompt:**\n> ${pf.originalPrompt}`;

        // Mirror approval-manager.createActionProposal here so we don't pull
        // in approval-manager and risk an import cycle.
        ...
        await prisma.chatAction.create({
          data: {
            ...
            plan: { title: previewTitle, description: previewDesc },
            actions: [
              {
                ...
                description: isHe
                  ? `קבע את התמונה הזו כתמונה הראשית של הפוסט #${pf.postId}.`
                  : `Set this image as the featured image of post #${pf.postId}.`,
              },
            ],
            rollbackData: [],
      ...
      console.warn('[ChatAction] Failed to create follow-up image-preview proposal:', followUpErr.message);
    }
    ...
    await prisma.chatMessage.create({
      data: {
        conversationId: chatAction.conversationId,
        role: 'ASSISTANT',
        content: `### ${t.actionCompleted}\n\n${summary}${longRunningNotice}${verificationNote}${previewExtra}${assistedExtra}${rollbackTail}`,
      },
    });
    ...
    // Goodwill refund: when an action fails (any step throws), refund every
    // AI-GCoin debit tagged with this chatActionId. The user shouldn't pay
    // for a workflow we couldn't complete - even if some sub-steps did run,
    // the user-visible outcome is "this didn't work". We let the user know
    // we covered it ("we didn't charge you for this attempt") which makes
    // failures feel less punitive and keeps trust intact.
    let refundLine = '';
    try {
      const refund = await refundChatActionCredits(chatActionId, chatAction.accountId, 'action_failed');
      if (refund?.success && refund.refunded > 0) {
        refundLine = lang === 'HE'
          ? `\n\n💚 **כמחווה:** לא חייבנו אותך על ${refund.refunded} AI-GCoins שצברה הפעולה הזו. הקרדיטים יוחזרו לחשבון מיידית - תוכל לנסות שוב מתי שתרצה.`
          : `\n\n💚 **As a courtesy:** we didn't charge you for the ${refund.refunded} AI-GCoins this action consumed. The credits are refunded to your account right away - feel free to try again whenever you're ready.`;
      } else {
        // No tracked debits to refund (the action failed before any AI call
        // burned credit). Still acknowledge it - the user shouldn't have to
        // wonder if they were charged.
        refundLine = lang === 'HE'
          ? `\n\n💚 **כמחווה:** לא חייבנו אותך על הניסיון הזה.`
          : `\n\n💚 **As a courtesy:** you weren't charged for this attempt.`;
      }
    } catch (refundErr) {
      // Don't let a refund-bookkeeping error obscure the original failure.
      console.warn('[ChatAction] Refund failed for', chatActionId, '-', refundErr.message);
    }
    ...
    await prisma.chatMessage.create({
      data: {
        conversationId: chatAction.conversationId,
        role: 'ASSISTANT',
        content: `### ${t.actionFailed}\n\n❌ ${t.error}: ${err.message}\n\n${t.completedXofY.replace('{done}', results.length).replace('{total}', chatAction.actions.length)}${refundLine}\n\n> ${t.rollbackPartialHint}`,
      },
    });
...
/**
 * Map a 2-letter site contentLanguage code to the human-readable name we want
 * to inject into the image generation prompt. Falls back to the locale string
 * when the code isn't recognized so the model still gets some signal.
 */
function languageNameFromSite(site) {
  const raw = (site?.contentLanguage || site?.language || site?.locale || '').toString().toLowerCase().trim();
  if (!raw) return null;
  const norm = raw.split(/[-_]/)[0];
  const map = {
    en: 'English',
    he: 'Hebrew',
    iw: 'Hebrew',
...
/**
 * Wrap the user's image prompt with a directive forcing any text rendered
 * INSIDE the image to use the site's language script. Nano Banana otherwise
 * defaults to English text on hero/banner images, which looks broken on
 * Hebrew / Arabic / etc. sites. Keeps the rest of the prompt intact - the
 * directive only controls in-image typography.
 */
function applyImageLanguageDirective(prompt, site) {
  const lang = languageNameFromSite(site);
  if (!lang || /english/i.test(lang)) return prompt;
  // RTL hint helps Nano Banana render Hebrew / Arabic correctly.
  const rtl = ['Hebrew', 'Arabic'].includes(lang) ? ' Use right-to-left (RTL) text direction.' : '';
  return [
    `LANGUAGE CONSTRAINT: Any visible text rendered inside the image (titles, labels, signs, watermarks, headlines, captions, UI elements) MUST be written in ${lang}, using natural ${lang} typography and a font that supports the ${lang} script.${rtl} If the prompt below references text in another language, transliterate or translate that text into ${lang} before rendering. Do NOT render Latin/English text inside the image unless the prompt explicitly says so.`,
    '',
    prompt,
  ].join('\n');
}
...
// ─── Cross-step argument resolution ───────────────────────────────────
//
// Multi-step plans frequently chain steps together: "generate an image, then
// insert it into the post body". The AI proposes them as separate tool calls
// inside the same propose_action plan, and historically expected the system
// to wire the outputs of one step into the inputs of the next. Without an
// explicit resolver, the second step crashed because mediaId/imageUrl was
// missing.
//
// Two resolution strategies, applied in this order:
//
//   1. Explicit template substitution. The AI can write `{{steps[1].mediaId}}`
//      or `{{prev.url}}` anywhere in args; we walk the structure and replace
//      those tokens against `priorResults`. `prev.*` resolves against the
//      most recent successful step. `steps[N].*` is 1-indexed.
//
//   2. Heuristic auto-fill for known image-consumer tools. If the AI omitted
//      mediaId/imageUrl on `wp_insert_image_in_content`, `wp_set_featured_image`,
//      or `wp_update_media`, look back through `priorResults` for the most
//      recent successful generate_image / wp_upload_media and pull mediaId
//      (and url for the insert tool) from there.
//
// Returns a NEW action object - never mutates the input.
const IMAGE_PRODUCERS = new Set(['generate_image', 'wp_upload_media']);
const IMAGE_CONSUMERS_NEEDING_MEDIA = new Set([
  'wp_insert_image_in_content',
  'wp_set_featured_image',
  'wp_update_media',
]);

function findMostRecentSuccessful(priorResults, predicate) {
  for (let i = priorResults.length - 1; i >= 0; i--) {
    const r = priorResults[i];
    if (r?.success && predicate(r)) return r;
  }
  return null;
}

function substituteTemplates(value, priorResults) {
  if (typeof value === 'string') {
    // {{prev.foo.bar}} or {{steps[2].mediaId}}
    return value.replace(/\{\{\s*([\w.[\]]+)\s*\}\}/g, (_, path) => {
      const segments = path.replace(/\[(\d+)\]/g, '.$1').split('.');
      let root;
      if (segments[0] === 'prev') {
        root = findMostRecentSuccessful(priorResults, () => true);
        segments.shift();
      } else if (segments[0] === 'steps' && segments[1] != null) {
        const idx1 = parseInt(segments[1], 10) - 1; // 1-indexed
        root = priorResults[idx1];
        segments.splice(0, 2);
      } else {
        return ''; // unrecognized root - drop the placeholder
      }
      let cur = root?.result ?? root ?? null;
  ...
  if (Array.isArray(value)) return value.map((v) => substituteTemplates(v, priorResults));
  if (value && typeof value === 'object') {
    const out = {};
    for (const [k, v] of Object.entries(value)) out[k] = substituteTemplates(v, priorResults);
    return out;
  }
  return value;
}

export function resolveCrossStepArgs(action, priorResults) {
  if (!action) return action;

  // (1) Explicit template substitution applied to every arg.
  const args = substituteTemplates(action.args || {}, priorResults);

  // (2) Heuristic auto-fill for image-consumer tools that came in without a
  // mediaId. Only applies when the prior chain produced an image - we check
  // that first to avoid clobbering legitimate "use a different mediaId from
  // wp_get_media" workflows.
  if (IMAGE_CONSUMERS_NEEDING_MEDIA.has(action.tool)) {
    const hasMediaRef = args.mediaId != null && args.mediaId !== ''
      || (action.tool === 'wp_insert_image_in_content' && args.imageUrl);
    if (!hasMediaRef) {
      const producer = findMostRecentSuccessful(
        priorResults,
        (r) => IMAGE_PRODUCERS.has(r.tool) && (r.result?.mediaId || r.result?.id),
      );
      if (producer) {
        const producedMediaId = producer.result?.mediaId ?? producer.result?.id;
        const producedUrl = producer.result?.url ?? producer.result?.source_url ?? null;
        if (producedMediaId != null) args.mediaId = producedMediaId;
        if (action.tool === 'wp_insert_image_in_content' && !args.imageUrl && producedUrl) {
          args.imageUrl = producedUrl;
        }
        // If the AI didn't supply alt text either, inherit from the producer
        // so the inserted image is still accessible / SEO-friendly.
        if (!args.alt && producer.result?.alt) args.alt = producer.result.alt;
      }
    }
  }
