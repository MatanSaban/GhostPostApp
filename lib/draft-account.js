...
export async function purgeDraftUserByEmail(email) {
  ...
  // Never wipe a user who's already verified their email or completed
  // registration — they own the address even if they never finished
  // payment. Caller (/api/auth/register) blocks the new registration
  // upstream; this is just defense-in-depth in case a different code path
  // calls purge directly.
  if (existing && existing.registrationStep !== 'COMPLETED' && !existing.emailVerified) {
