...
import { scheduleInterviewPostCompletion } from '@/lib/interview/post-completion';
...
export async function completeInterview(params, context) {
  ...
  try {
    // Re-read interview from DB to ensure we have the absolute latest responses
    const freshInterview = await context.prisma.userInterview.findUnique({
      where: { id: context.interview.id },
    });
    
    const interview = freshInterview || context.interview;
    ...
    try {
      ...
      if (strategyResult.success) {
        ...
        // Track Ai-GCoins usage for SEO strategy generation
        if (context.accountId) {
          await trackAIUsage({
            accountId: context.accountId,
            userId: context.userId,
            siteId: context.siteId || interview.siteId || null,
    ...
    if (createAccount && !siteId && responses.websiteUrl) {
      ...
      // Prefer the crawled URL (has correct protocol after redirect detection)
      // Fall back to raw user input if crawl data isn't available
      const siteUrl = externalData.crawledData?.url || responses.websiteUrl;
    ...
    if (siteId) {
      ...
      if (responses.businessInfo) updateFields.businessInfo = responses.businessInfo;
      if (responses.contentLanguage) updateFields.contentLanguage = responses.contentLanguage;
      if (responses.targetLocations) updateFields.targetLocations = responses.targetLocations;
      if (responses.writingStyle) updateFields.writingStyle = responses.writingStyle;
      if (responses.internalLinksPer1000Words != null) updateFields.internalLinksPer1000Words = responses.internalLinksPer1000Words;
      if (responses.websitePlatform) updateFields.platform = responses.websitePlatform;
      ...
      try {
        ...
        console.log(`[CompleteInterview] responses.keywords type: ${typeof keywordsToSave}, isArray: ${Array.isArray(keywordsToSave)}, value:`, JSON.stringify(keywordsToSave).slice(0, 200));
        ...
        console.log(`[CompleteInterview] Extracted ${keywordStrings.length} keyword strings for site ${siteId}`);
        ...
        if (keywordStrings.length > 0) {
          ...
          let newKeywords = keywordStrings
          ...
          // Respect the plan's maxKeywords cap. If adding all of them would
          // overflow, keep only as many as fit. Better to drop a few than to
          // fail the whole interview completion.
          if (newKeywords.length > 0 && context.accountId) {
            try {
              const usage = await getAccountUsage(context.accountId, 'maxKeywords');
              if (usage.remaining !== null && newKeywords.length > usage.remaining) {
                ...
                console.log(
                  `[CompleteInterview] Truncated keywords to fit plan maxKeywords cap ` +
                  `(${usage.used}/${usage.limit}, dropped ${skipped}).`,
                );
              }
            } catch (capErr) {
              console.warn('[CompleteInterview] keyword capacity check failed:', capErr.message);
            }
          }
      ...
      try {
        ...
        // Track remaining competitor headroom for this site so we don't
        // exceed the plan's maxCompetitors cap. Reactivations don't consume
        // capacity since they were already counted.
        let remainingCap = Infinity;
        if (competitorUrls.length > 0 && context.accountId) {
          try {
            const cap = await getSiteCompetitorCapacity(context.accountId, siteId);
            remainingCap = cap.remaining;
          } catch (capErr) {
            console.warn('[CompleteInterview] competitor capacity check failed:', capErr.message);
          }
        }
          ...
          for (const url of competitorUrls) {
            try {
              ...
              if (!existing) {
                if (remainingCap <= 0) {
                  console.log(
                    `[CompleteInterview] Skipping competitor ${domain} - maxCompetitors cap reached.`,
                  );
                  continue;
                }
    ...
    // Kick off post-completion work (entity population + first agent run)
    // after the response is sent
    if (siteId) {
      scheduleInterviewPostCompletion({
        siteId,
        accountId: newAccountId || context.accountId,
        userId: context.userId,
        entitiesSelection: responses.entitiesSelection ?? responses['entities-selection-injected'] ?? null,
      });
    }
