...
export async function crawlWebsite(params, context) {
  ...
  // Check for cached crawl data on the Site to avoid re-crawling with AI.
  // Cached display fields (description, category, services, targetAudience)
  // are language-specific now — they're written in whatever the requesting
  // user's UI locale was. So the cache is only safe to reuse when its
  // displayLocale matches the current request. If they differ, fall through
  // and re-extract so the chat doesn't end up bilingual again.
  const requestedLocale = (params.userLocale || context.userLocale || '').toLowerCase() || null;
  const CACHE_DURATION_DAYS = 30;
  if (context.siteId && context.prisma) {
    try {
      const site = await context.prisma.site.findUnique({
        where: { id: context.siteId },
        select: { crawledData: true, crawledAt: true },
      });

      if (site?.crawledData && site?.crawledAt) {
        const daysSinceCrawl = (Date.now() - new Date(site.crawledAt).getTime()) / (1000 * 60 * 60 * 24);
        const cachedLocale = (site.crawledData.displayLocale || '').toLowerCase() || null;
        const localeMismatch = requestedLocale && cachedLocale && cachedLocale !== requestedLocale;
        // Older cache entries (pre-locale plumbing) have no displayLocale.
        // Treat those as a mismatch when a userLocale was requested, so the
        // first user under a given locale always gets a fresh extraction.
        const cacheMissingLocale = requestedLocale && !cachedLocale;

        if (daysSinceCrawl < CACHE_DURATION_DAYS && !localeMismatch && !cacheMissingLocale) {
          console.log(`[CrawlWebsite Handler] Using cached crawl data (${Math.round(daysSinceCrawl)} days old, locale=${cachedLocale || 'n/a'})`);
          
          const cachedData = site.crawledData;
          
          // Save cached data to interview externalData for consistency
          if (context.interview) {
            ...
            await context.prisma.userInterview.update({
              ...
              data: {
                externalData: {
                  ...existingData,
                  crawledData: cachedData,
                  _fromCache: true,
                  _cachedAt: site.crawledAt,
                },
              },
            });
          }
          ...
          return {
            ...
            data: {
              ...
              phones: cachedData.phones,
              socialLinks: cachedData.socialLinks,
              seoScore: cachedData.technicalAudit?.healthScore,
              hasSitemap: cachedData.hasSitemap,
              sitemapUrlCount: cachedData.sitemapUrls?.length || 0,
              technicalAudit: cachedData.technicalAudit,
            },
          };
        } else if (localeMismatch || cacheMissingLocale) {
          console.log(`[CrawlWebsite Handler] Cache locale mismatch (cached=${cachedLocale || 'unknown'}, requested=${requestedLocale}); re-crawling for fresh translation`);
        } else {
          console.log(`[CrawlWebsite Handler] Cached data expired (${Math.round(daysSinceCrawl)} days old), re-crawling`);
        }
      }
    } catch (err) {
      console.error("[CrawlWebsite Handler] Error checking cached data:", err.message);
      // Continue with fresh crawl on error
    }
  }
  ...
  try {
    ...
    // Use the comprehensive crawl function. userLocale propagates from the
    // wizard (params.userLocale) or the executor's context.userLocale into
    // the AI extraction step so business description / category / services /
    // target audience are written in the user's UI language. This keeps the
    // interview chat monolingual when the website itself is in a different
    // language from the chat (e.g. English site, Hebrew chat).
    const crawlResult = await crawlWebsiteFunction(url, { userLocale: requestedLocale });
    ...
    if (context.interview && context.prisma) {
      ...
      // Detect language first so we can derive country from it
      const detectedLanguage = aiData.detectedLanguage || crawlResult.pageData?.language || "en";

      // Map language to most likely target country
      // Used as default pre-selection for the target location question
      const languageToCountryMap = {
        he: 'IL',      // Hebrew → Israel
        ar: 'AE',      // Arabic → UAE (most common for web)
        ja: 'JP',      // Japanese → Japan
        ko: 'KR',      // Korean → South Korea
        zh: 'CN',      // Chinese → China
        ru: 'RU',      // Russian → Russia
        pt: 'BR',      // Portuguese → Brazil (most speakers)
        ...
        hi: 'IN',      // Hindi → India
        en: 'worldwide', // English → Worldwide
      };
      const detectedCountry = languageToCountryMap[detectedLanguage] || 'worldwide';
      ...
      const crawledData = {
        ...
        // Language - the SITE's own language (prefer AI-detected, fallback to HTML lang)
        language: detectedLanguage,
        // displayLocale = the locale the human-readable AI fields above were
        // written in. Used by the cache check on the next crawl call so we
        // don't serve, say, Hebrew description text to an English user.
        // Null when no locale was supplied (server-side / older callers).
        displayLocale: requestedLocale || null,
        
        // Detected target country based on language (for pre-selection)
        detectedCountry,
      ...
      // Also save crawled data to the Site for caching across interviews
      if (context.siteId) {
        try {
          ...
          console.log("[CrawlWebsite Handler] Cached crawl data to Site:", context.siteId);
        } catch (siteErr) {
          console.error("[CrawlWebsite Handler] Error caching to site:", siteErr.message);
          // Non-fatal - interview data is already saved
        }
      }
    } else {
      console.log(
