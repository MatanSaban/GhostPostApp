...
export async function generateKeywords(params, context) {
  try {
    ...
    // Filter out keywords with dynamic placeholders like [עיר], [שם], [מיקום], etc.
    const hasDynamicPlaceholder = (keyword) => /\[.*?\]/.test(keyword);
    ...
    crawledKeywords.forEach((kw, index) => {
      ...
      if (!seenKeywords.has(normalizedKw) && normalizedKw.length > 2 && !hasDynamicPlaceholder(kw)) {
    ...
    if (aiResult.success && aiResult.data) {
      ...
      aiKeywords.forEach(kw => {
        ...
        if (!seenKeywords.has(normalizedKw) && normalizedKw.length > 2 && !hasDynamicPlaceholder(kw.keyword)) {
          seenKeywords.add(normalizedKw);
          keywords.push(kw);
        }
      });
    ...
    if (context.accountId) {
      // Get websiteUrl from context or interview data
      const websiteUrl = context.responses?.websiteUrl || 
                         context.externalData?.crawledData?.url ||
                         context.interview?.responses?.websiteUrl ||
                         context.interview?.externalData?._rawCrawlResult?.url;
      ...
      await trackAIUsage({
        ...
        description: `Generated ${keywords.length} keyword suggestions`,
        metadata: {
          keywordsFromWebsite: crawledKeywords.length,
          keywordsFromAI: keywords.length - crawledKeywords.length,
          totalKeywords: keywords.length,
          businessName: businessInfo.businessName,
          websiteUrl,
          descriptionKey: 'generatedKeywords',
          descriptionParams: { count: keywords.length },
        },
      });
    }
