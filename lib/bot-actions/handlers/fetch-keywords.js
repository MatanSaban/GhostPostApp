 ...
 * Enriches with real search volume from Google Ads Keyword Planner when available.
 */

import { generateKeywords } from '@/lib/ai/interview-ai.js';
import { batchGetSearchVolume, isGoogleAdsConfigured, getLanguageId } from '@/lib/google-ads.js';
...
export async function fetchKeywords(params, context) {
  ...
  try {
    ...
    // Enrich with real search volume from Google Ads Keyword Planner
    if (keywords.length > 0 && isGoogleAdsConfigured()) {
      try {
        const kwStrings = keywords.map(k => k.keyword);
        const volumeData = await batchGetSearchVolume(kwStrings);
        if (volumeData && volumeData.size > 0) {
          for (const kw of keywords) {
            const data = volumeData.get(kw.keyword.toLowerCase().trim());
            if (data) {
              kw.searchVolume = data.avgMonthlySearches;
              kw.realVolume = true; // Flag that this is real data
            }
          }
        }
      } catch (enrichErr) {
        console.error('Google Ads enrichment failed (using AI estimates):', enrichErr.message);
      }
    }
