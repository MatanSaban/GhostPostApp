...
/**
 * Aggregate raw per-keyword search hits into a ranked competitor list.
 * Pure function - safe to call repeatedly on the growing allCompetitors
 * array for partial progress snapshots (autoSelect:false) and once at the
 * end for the final list (autoSelect:true → top 5 pre-checked).
 *
 * SCORING FORMULA:
 ...
 *
 * @param {Array} allCompetitors - Raw competitor hits from all keyword searches
 * @param {Object} options
 * @param {number} [options.limit=10] - Max competitors to return
 * @param {boolean} [options.autoSelect=false] - When true, auto-select the top 5
 * @returns {Array} Ranked, decorated competitor list
 */
function aggregateAndRank(allCompetitors, { limit = 10, autoSelect = false } = {}) {
  ...
  return Array.from(competitorScores.values())
    ...
    .slice(0, limit) // Top 10 by default
    .map((comp, index) => ({
      name: comp.name,
      url: comp.url,
      domain: comp.domain,
      ...
      autoSelected: autoSelect ? index < 5 : false, // Auto-select top 5 (final pass only)
      source: comp.source,
      verified: comp.verified,
    }));
}
...
export async function findCompetitors(params, context) {
  ...
  // ── Progressive search state (client polls externalData.competitorSearch) ──
  const totalKeywords = keywordsToSearch.length;
  const searchStartedAt = new Date().toISOString();
  // Shared results array, filled incrementally by each keyword search
  const allCompetitors = [];
  let completedKeywords = 0;
  // Serialized write chain - partial writes never overlap each other
  let writeChain = Promise.resolve();
  const canWriteProgress = !!(context.interview && context.prisma);

  // Fresh-read externalData then merge in the competitorSearch blob.
  // Fresh read is required: Prisma/Mongo replaces the whole JSON column,
  // and concurrent actions may have written since request start.
  const writeCompetitorSearch = async (competitorSearch) => {
    const fresh = await context.prisma.userInterview.findUnique({
  ...
  // Enqueue a partial progress write. Snapshots count + ranking at enqueue
  // time; failures only warn - they must never break the search itself.
  const queuePartialWrite = () => {
    if (!canWriteProgress) return;
    const partialCompetitors = aggregateAndRank(allCompetitors, { autoSelect: false });
    const completed = completedKeywords;
    writeChain = writeChain
      .then(() => writeCompetitorSearch({
        status: 'running',
        totalKeywords,
      ...
      .catch((err) => {
        console.warn('[FindCompetitors] Partial progress write failed:', err?.message || err);
      });
  };
  ...
  try {
    ...
    // Write initial progress so the client can start polling right away
    if (canWriteProgress) {
      try {
        await writeCompetitorSearch({
          status: 'running',
          totalKeywords,
          completedKeywords: 0,
          partialCompetitors: [],
        ...
        console.warn('[FindCompetitors] Initial progress write failed:', err?.message || err);
      }
    }
    ...
    const parallelResults = await Promise.all(
      keywordsToSearch.map((keyword) =>
        searchCompetitorsForKeyword(
          ...
          // Stream progress: append this keyword's results and enqueue a
          // serialized partial write so the client sees competitors early
    ...
    // Rebuild in keyword order so the final ranking matches the previous
    // behavior exactly (completion order only affects partial snapshots)
    ...
    // All searches done - let every queued partial write land BEFORE the
    // final write, so a straggler can't flip status back to "running"
    await writeChain;

    // Final ranking with auto-select of the top 5
    const rankedCompetitors = aggregateAndRank(allCompetitors, { autoSelect: true });
    ...
    // Persist 'done' from the handler itself (not only via the caller's
    // storeInExternalData merge) so invocation paths that ignore the return
    // value — e.g. flow-engine auto-actions — can't leave the status stuck
    // at 'running'. Includes the final ranked list so a poller terminating
    // on this write still gets complete data with autoSelected flags.
    if (canWriteProgress) {
      try {
        await writeCompetitorSearch({
          status: 'done',
          ...
          partialCompetitors: decoratedCompetitors,
          startedAt: searchStartedAt,
          updatedAt: new Date().toISOString(),
          finishedAt: new Date().toISOString(),
        });
      } catch (err) {
        console.warn('[FindCompetitors] Done-status write failed:', err?.message || err);
      }
    }
    ...
    // Best-effort: mark the progressive search failed so client polling stops
    if (canWriteProgress) {
      try {
        await writeChain;
        await writeCompetitorSearch({
        ...
        console.warn('[FindCompetitors] Failed-status write failed:', writeError?.message || writeError);
      }
    }
    return {
      success: false,
