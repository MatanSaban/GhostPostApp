/**
 * Google OAuth utilities for GhostSEO platform
 ...
 * Uses BASE_URL (runtime env var) for server-side OAuth
 * NEXT_PUBLIC_* vars are inlined at build time - not suitable for server-side runtime
 */
function getBaseUrl() {
  // BASE_URL is read at runtime (not build time like NEXT_PUBLIC_*)
  // Set this in Vercel env vars for production and .env for local dev
...
function getOAuthConfig() {
  ...
  console.log('[Google OAuth] Using redirect URI:', `${baseUrl}/api/auth/google/callback`);
...
export async function exchangeCodeForTokens(code) {
  ...
  console.log('[Google OAuth] Exchanging code for tokens with redirect_uri:', redirectUri);
...
export async function getGoogleUserInfo(accessToken) {
  ...
  // Log the raw picture URL for debugging Workspace vs personal accounts
  console.log('[Google OAuth] User info received:', {
    email: data.email,
    picture: data.picture,
    hasPicture: !!data.picture,
    isWorkspace: data.hd ? `domain: ${data.hd}` : 'personal account',
  });
  ...
  return {
    ...
    name: data.name,
    picture: data.picture,
    hostedDomain: data.hd, // Google Workspace domain if applicable
  };
}
