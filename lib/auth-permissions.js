 ...
 * @returns {Promise<{authorized: boolean, member: Object|null, error: string|null, isSuperAdmin: boolean}>}
 */
export async function getCurrentAccountMember() {
  try {
    ...
    if (!userId) {
      return { authorized: false, member: null, error: "Unauthorized", isSuperAdmin: false };
    }
    ...
    if (!user) {
      return { authorized: false, member: null, error: "User not found", isSuperAdmin: false };
    }
    ...
    // If no account selected, use the first active membership
    let accountId = user.lastSelectedAccountId;

    if (!accountId && user.accountMemberships.length > 0) {
    ...
    if (!accountId) {
      return { authorized: false, member: null, error: "No account selected", isSuperAdmin: false };
    }
    ...
    return { authorized: true, member, error: null, isSuperAdmin: false };
  } catch (error) {
    console.error("Auth error:", error);
    return { authorized: false, member: null, error: "Authentication failed", isSuperAdmin: false };
  }
}
