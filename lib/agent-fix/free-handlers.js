...
// ─── numericSlugSuffix: rename slug, 301 the old URL, rewrite inbound links ─
// Per page: rename slug → 301 old→new (CMS + platform DB) → search-replace links so we don't rely on the 301 hop. Shopify has no link-rewrite endpoint, so step 3 is skipped there.
...
export async function applySlugSuffixCleanupFix(insight, site, itemIndices = null) {
  const pages = insight.data?.pages || [];
  if (pages.length === 0) {
    return { success: false, results: [], summary: 'No pages to fix' };
  }
  ...
  for (const i of indices) {
    const page = pages[i];
    if (!page?.url || !page?.suggestedSlug || !page?.suggestedUrl) {
      results.push({ index: i, url: page?.url || null, status: 'skipped', reason: 'Missing slug/url data' });
      continue;
    }
    ...
    try {
      ...
      await cms.updatePost(site, resolved.postType, resolved.externalId, {
        slug: page.suggestedSlug,
      });

      let redirectStatus = 'skipped';
      if (!page.alreadyRedirected) {
        const fromPath = pathOf(page.url);
        const toPath = pathOf(page.suggestedUrl);
        try {
          ...
          redirectStatus = 'created';
        } catch (redirErr) {
          // Non-fatal: rename succeeded, user can add the redirect manually.
          redirectStatus = `failed: ${redirErr.message}`;
        }
      }
      ...
      if (site.platform !== 'shopify') {
        try {
          const out = await cms.searchReplaceLinks(site, page.url, page.suggestedUrl);
          linksUpdated = out?.updated || 0;
        } catch {
          // Non-fatal — the 301 covers correctness if rewrite fails.
        }
      }
      ...
      results.push({
        index: i,
        url: page.url,
        newUrl: page.suggestedUrl,
        postId: resolved.externalId,
        status: 'fixed',
        reason: `Slug renamed; redirect ${redirectStatus}; ${linksUpdated} link(s) rewritten`,
      });
    } catch (err) {
      results.push({
        index: i,
  ...
  return {
    success: fixed > 0,
    results,
    summary: `Cleaned slug suffix on ${fixed}/${indices.length} page(s)`,
  };
}

export async function applyRescanCompetitorsFix(insight, site, itemIndices = null) {
  const competitors = insight.data?.competitors || [];
  const indices = itemIndices && itemIndices.length > 0 ? itemIndices : competitors.map((_, i) => i);
  const domains = indices.map(i => competitors[i]?.domain).filter(Boolean);
  ...
  const status = updated.count > 0 ? 'fixed' : 'skipped';
  return {
    success: updated.count > 0,
    // One result per requested row so per-item UI matching (index/url) works.
    results: indices.map(i => ({
      index: i,
      url: competitors[i]?.domain || null,
      status,
      ...(status === 'skipped' ? { reason: 'No matching competitors to rescan' } : {}),
    })),
