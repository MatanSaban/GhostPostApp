...
import { logAIUsage, getOperationCreditCost } from './credits.js';
 ...
 * Mirrors the helper in lib/ai/gemini.js - kept local so service.js doesn't
 * depend on internal exports of gemini.js.
 */
async function assertCreditsForOperation({ accountId, operation }) {
  if (!accountId) return;
  const required = await getOperationCreditCost(operation);
 ...
 */
export async function chat({ messages, systemPrompt, botActions = [], operation = 'INTERVIEW_CHAT', metadata = {}, accountId, userId, siteId }) {
  try {
    ...
    logAIUsage({
      ...
      metadata: {
        ...
        hasToolCall: !!(result.toolCalls && result.toolCalls.length > 0),
        ...metadata,
      },
    });
 ...
 * @returns {Promise<{content: string, toolCall: Object|null}>}
 */
export async function continueWithToolResult({ messages, systemPrompt, toolCall, toolResult, botActions = [], accountId, userId, siteId }) {
  ...
  return await chat({ messages: updatedMessages, systemPrompt, botActions, accountId, userId, siteId });
}
