...
export const AI_PRICING = {
  ...
  // Embedding models. Pricing is per 1M INPUT tokens only - embedding models
  // produce vectors, not tokens, so there is no output cost. The Vercel AI SDK
  // calls them with usage = { inputTokens, outputTokens: 0 } (or missing
  // outputTokens), and calculateTokenCost handles outputPer1M=0 correctly.
  // Used by cluster-auto-map, cluster-cannibalization-preflight, and
  // cluster-discovery for semantic similarity / cannibalization detection -
  // the model converts post titles + content excerpts into 768-dim vectors
  // that we compare with cosineSimilarity to decide if two posts are
...
// When an unknown model ID arrives at getModelPricing(), we bill it at this
// rate rather than hard-failing - keeps usage logging robust against typos and
// new model launches. Set to the latest production text model so any future
// "gemini-3.2-pro" etc. is over-charged (safe-for-the-platform direction)
// rather than under-charged. Update this whenever Google ships a newer flagship.
