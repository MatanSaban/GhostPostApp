 ...
 * - Tracking Ai-GCoins usage to the database
...
import { AI_OPERATIONS, tokensToCredits, getOperationConfig, getOperationCreditCost } from './credits.js';

/**
 * After a debit is recorded, check if period allocation is overflowed
 * and deplete one-time Ai-GCoin packs (FIFO by purchase date).
 *
...
async function depleteOneTimePacks(accountId, creditsJustUsed) {
  try {
    ...
    // Fetch all active Ai-GCoin add-on purchases in one query
...
export async function trackAIUsage({
  ...
  console.log('[CreditsService] trackAIUsage called with:', { accountId, operation, userId, creditsMultiplier });
  ...
  // Calculate credits: fetch dynamic price from DB, multiply by multiplier
  const baseCreditCost = await getOperationCreditCost(operation);
  const creditsUsed = baseCreditCost * creditsMultiplier;

  // Refuse to record a DEBIT if the account is already over its limit.
  // This is defense-in-depth - callers should also pre-flight via `enforceCredits`
  // (or use the gemini.js helpers, which do that for them) so the LLM doesn't
  // even run. But if they forget, this prevents silent over-billing.
  const limitCheck = await enforceCredits(accountId, creditsUsed);
  if (!limitCheck.allowed) {
    console.warn(
      `[CreditsService] Refusing DEBIT for ${operation} (${creditsUsed} credits) - account ${accountId} is over its Ai-GCoins limit. ` +
      `used=${limitCheck.usage?.used}, limit=${limitCheck.usage?.limit}`
    );
    return {
      success: false,
      code: 'INSUFFICIENT_CREDITS',
      error: limitCheck.error || 'Insufficient Ai-GCoins',
  ...
  try {
    // Update account credits with retry for write conflicts
    const MAX_RETRIES = 3;
    let updatedAccount;
    ...
    // Deplete one-time Ai-GCoin packs if period allocation is overflowed
...
/**
 * Refund AI-GCoins consumed inside a single ChatAction execution.
 *
 * Looks up every DEBIT log entry tagged with `metadata.chatActionId === chatActionId`,
 * sums their amounts, decrements `aiCreditsUsedTotal` by that sum (never going
 * below zero), and writes a single REFUND log entry that points back at the
 * failed chat action. Idempotent: if a REFUND for this chatActionId already
 * exists in the log, the second call is a no-op.
 *
 * Returns `{ success: boolean, refunded: number, debits: number }`.
 *
 * @param {string} chatActionId - The ChatAction whose spend should be refunded
 * @param {string} accountId    - The owning account
 * @param {string} [reason]     - Short reason for the refund (defaults to action_failed)
 */
export async function refundChatActionCredits(chatActionId, accountId, reason = 'action_failed') {
  if (!chatActionId || !accountId) return { success: false, refunded: 0, debits: 0 };

  try {
    // Idempotency guard - never double-refund the same action.
    const existing = await prisma.aiCreditsLog.findFirst({
      where: { accountId, type: 'CREDIT', source: 'CHAT_ACTION_REFUND', metadata: { is: { chatActionId } } },
      select: { id: true, amount: true },
    });
    if (existing) {
      return { success: true, refunded: existing.amount, debits: 0, alreadyRefunded: true };
    }

    // Sum every debit metadata-tagged with this chatActionId. Tracking happens
    // in trackAIUsage() callers - we expect them to stamp metadata.chatActionId
    // when running inside an action's execute path. If the caller forgot to
    // tag, we won't refund those (the next-best signal would be a time window
    // around the action's executedAt, but that's heuristic and risks refunding
    // unrelated activity by the user during the same minute).
    const debits = await prisma.aiCreditsLog.findMany({
      where: { accountId, type: 'DEBIT', metadata: { is: { chatActionId } } },
      select: { id: true, amount: true },
    });
    const totalToRefund = debits.reduce((sum, d) => sum + (d.amount || 0), 0);
    if (totalToRefund <= 0) {
      return { success: true, refunded: 0, debits: 0 };
    }

    // Decrement aiCreditsUsedTotal but never below zero. Read-then-write rather
    // than `decrement` because Prisma's decrement can underflow into negative.
    const account = await prisma.account.findUnique({
      where: { id: accountId },
      select: { aiCreditsUsedTotal: true },
    });
    const currentUsed = account?.aiCreditsUsedTotal || 0;
    const newUsed = Math.max(0, currentUsed - totalToRefund);
    await prisma.account.update({
      where: { id: accountId },
      data: { aiCreditsUsedTotal: newUsed },
    });

    // Single REFUND log entry summarising the operation. Keeps the per-debit
    // entries intact for forensic purposes (we don't delete them - the account
    // history reads correctly: original debits, then a single refund).
    await prisma.aiCreditsLog.create({
      data: {
        ...
        balance: 0,
        source: 'CHAT_ACTION_REFUND',
        description: `Refund for failed chat action`,
        metadata: { chatActionId, reason, debitCount: debits.length },
      },
    });

    console.log(`[CreditsService] Refunded ${totalToRefund} credits across ${debits.length} debit(s) for chat action ${chatActionId}.`);
    return { success: true, refunded: totalToRefund, debits: debits.length };
  } catch (error) {
    console.error('[CreditsService] refundChatActionCredits failed:', error.message);
    return { success: false, refunded: 0, debits: 0, error: error.message };
  }
}
