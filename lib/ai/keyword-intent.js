 ...
 */
export async function analyzeKeywordIntent(keyword, context = {}) {
 ...
 */
export async function analyzeKeywordIntentsBatch(keywords, context = {}) {
  ...
  if (keywords.length <= 3) {
    ...
    for (const kw of keywords) {
      const result = await analyzeKeywordIntent(kw, context);
...
Return the intent classification for each keyword.`,
    ...
    operation: 'KEYWORD_INTENT_BATCH',
