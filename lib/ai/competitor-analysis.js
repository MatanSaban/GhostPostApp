 ...
 */
export async function analyzeCompetitorTopics(content, title, context = {}) {
 ...
 * @param {Object} context - Optional context with accountId, userId, siteId
 * @returns {Promise<Object>} Content gaps and recommendations
 */
export async function identifyContentGaps(userContent, competitorContent, userTitle, competitorTitle, context = {}) {
  const schema = z.object({
 ...
 */
export async function generateSkyscraperOutline(competitorData, gapAnalysis, targetKeyword, context = {}) {
  const schema = z.object({
 ...
 */
export async function generateCompetitorSummary(content, title, context = {}) {
  const schema = z.object({
