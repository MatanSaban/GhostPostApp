...
const INTERVIEW_SYSTEM_PROMPT = `You are Ghost, a friendly and professional AI assistant for GhostSEO - an SEO content automation platform.
...
Respond in ${language === 'he' ? 'Hebrew' : 'English'}.`;
  ...
  try {
    const response = await generateTextResponse({
      ...
      metadata: { questionType: question.questionType, language },
    });
...
export async function generateKeywords(businessInfo, competitors = []) {
  ...
  // Build target audience context if available
  const targetAudienceContext = businessInfo.targetAudience 
...
Description: ${businessInfo.description || 'Not provided'}${targetAudienceContext}${existingKeywordsContext}
${competitors.length > 0 ? `Competitors: ${competitors.join(', ')}` : ''}

IMPORTANT: Read the description carefully to understand what TYPE of website this is (blog, agency, e-commerce, tool, etc.) and generate keywords that match that type.
- For a BLOG/content site: focus on informational keywords that readers would search for (e.g., "how to...", "guide to...", "best practices for...")
- For a SERVICE PROVIDER/agency: focus on service-related keywords (e.g., "marketing agency", "web development services")
- For an E-COMMERCE store: focus on product keywords (e.g., "buy ...", "... price", product names)
- For a SaaS/tool: focus on tool-related keywords (e.g., "... tool", "... software", "best ... for")
...
Important: Generate keywords that would help this business rank in search engines. Consider keywords the competitors might be using.`;
  ...
  try {
    const result = await generateStructuredResponse({
      ...
      temperature: 0.5,
      operation: 'GENERIC',
      metadata: { businessName: businessInfo.businessName, competitorsCount: competitors.length },
    });
...
Determine the writing style and provide recommendations for maintaining consistency.`;
  ...
  try {
    const result = await generateStructuredResponse({
      ...
      temperature: 0.4,
      operation: 'GENERIC',
      metadata: { samplesCount: contentSamples.length },
    });
...
Generate a comprehensive but actionable SEO strategy tailored to this business.`;
  ...
  try {
    const result = await generateStructuredResponse({
      ...
      temperature: 0.6,
      operation: 'GENERIC',
      metadata: { hasBusinessData: !!interviewData.businessInfo },
    });
