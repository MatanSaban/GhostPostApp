/**
 * Ai-GCoins Pricing System
 * 
 * This module defines the pricing for AI operations in the GhostSEO platform.
 * 
 * Pricing Formula:
 * 1 Ai-GCoin = 2,500 tokens
 * 
...
export const AI_OPERATIONS = {
  ...
  REGENERATE_MEDIA_FIELD: {
    key: 'REGENERATE_MEDIA_FIELD',
    name: 'Media Field Regenerate (alt/title/caption/description)',
    nameHe: 'יצירה מחדש של שדה מדיה',
    credits: 1,
    tokenValue: 2500,
    typicalUsage: 400,
    model: 'gemini-2.5-pro',
  ...
  AGENT_SUGGEST_TRAFFIC: {
    key: 'AGENT_SUGGEST_TRAFFIC',
    name: 'Traffic Improvement Suggestions',
    nameHe: 'הצעות לשיפור תנועה אורגנית',
    credits: 5,
    tokenValue: 12500,
    typicalUsage: 3000,
    model: 'gemini-2.5-pro',
  ...
  REGENERATE_MEDIA_IMAGE: {
    key: 'REGENERATE_MEDIA_IMAGE',
    name: 'Media Library Image Regenerate',
    nameHe: 'יצירה מחדש של תמונה בספריית מדיה',
    credits: 5,
    tokenValue: 12500,
    typicalUsage: 2000,
    model: 'gemini-2.5-pro',
  ...
  BACKLINK_LISTING: {
    key: 'BACKLINK_LISTING',
    name: 'Backlink Listing Generation',
    nameHe: 'יצירת רישום קישורים',
    credits: 1,
    tokenValue: 2500,
    typicalUsage: 500,
    model: 'gemini-2.5-pro',
  ...
  // Backlink Audit — toxic-link AI judgment. One batched LLM call evaluates
  // up to ~30 candidates per run; flat 2-credit cost reflects the heavier
  // structured reasoning vs. routine text generation.
  AGENT_INSIGHT_BACKLINK_TOXIC: {
    key: 'AGENT_INSIGHT_BACKLINK_TOXIC',
    name: 'Backlink Toxicity Analysis',
    nameHe: 'ניתוח רעילות קישורים',
    credits: 2,
    tokenValue: 5000,
    typicalUsage: 3000,
  ...
  // Embeddings - used as a cheap pre-filter before expensive reasoning calls
  // (cannibalization triage, semantic dedup, etc.). Billed as 1 credit per run.
  EMBEDDING: {
    key: 'EMBEDDING',
    name: 'Text Embedding',
    nameHe: 'וקטור משמעות (embedding)',
    credits: 1,
    tokenValue: 2500,
    typicalUsage: 1500,
    model: 'gemini-embedding-001',
  },
  CANNIBALIZATION_EMBEDDING: {
    key: 'CANNIBALIZATION_EMBEDDING',
    name: 'Cannibalization Embedding Pre-filter',
    nameHe: 'סינון Embeddings לקניבליזציה',
    credits: 1,
    tokenValue: 2500,
    typicalUsage: 2000,
    model: 'gemini-embedding-001',
  ...
  CLUSTER_DISCOVERY: {
    key: 'CLUSTER_DISCOVERY',
    name: 'Topic Cluster Discovery',
    nameHe: 'גילוי אשכולות נושאים',
    credits: 5,
    tokenValue: 12500,
    typicalUsage: 2000,
    model: 'gemini-2.5-pro',
  },
  CLUSTER_GAP_SUGGESTIONS: {
    key: 'CLUSTER_GAP_SUGGESTIONS',
    name: 'Cluster Gap Suggestions',
    nameHe: 'הצעות לפערי תוכן באשכול',
    credits: 5,
    tokenValue: 12500,
    typicalUsage: 2500,
    model: 'gemini-2.5-pro',
  ...
  CLUSTER_AUTO_MAP: {
    key: 'CLUSTER_AUTO_MAP',
    name: 'Cluster Auto-Map',
    nameHe: 'מיפוי אוטומטי לאשכול',
    credits: 1,
    tokenValue: 2500,
    typicalUsage: 800,
    model: 'gemini-embedding-001',
  ...
  CLUSTER_LINK_FIX: {
    key: 'CLUSTER_LINK_FIX',
    name: 'Cluster Internal-Link Fix',
    nameHe: 'תיקון קישור פנימי באשכול',
    credits: 10,
    tokenValue: 25000,
    typicalUsage: 4000,
    model: 'gemini-2.5-pro',
...
/**
 * Get operation config by key (static/hardcoded fallback)
...
/**
 * Get the credit cost for an operation, driven by the AiFeaturePricing DB table.
 * Falls back to the hardcoded AI_OPERATIONS value if no DB row exists.
 * @param {string} operationKey - Operation key
 * @returns {Promise<number>} Credit cost
 */
export async function getOperationCreditCost(operationKey) {
  try {
    const { getFeatureCreditCost } = await import('@/lib/actions/ai-pricing.js');
    return await getFeatureCreditCost(operationKey);
  } catch {
    // Fallback to hardcoded
    return (AI_OPERATIONS[operationKey] || AI_OPERATIONS.GENERIC).credits;
  }
}
