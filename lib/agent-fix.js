...
import { refreshAccessToken } from '@/lib/google-integration.js';
...
const CannibalizationFixSchema = z.object({
  ...
  pageAChanges: z.object({
    newTitle: z.string().describe('For DIFFERENTIATE: new SEO title. For other actions: the current/existing SEO title of page A as-is.'),
    newDescription: z.string().describe('For DIFFERENTIATE: new meta description. For other actions: the current/existing meta description of page A as-is.'),
    newFocusKeyword: z.string().describe('For DIFFERENTIATE: new unique focus keyword. For other actions: the current focus keyword or main keyword of page A.'),
  ...
  pageBChanges: z.object({
    newTitle: z.string().describe('For DIFFERENTIATE: new SEO title. For other actions: the current/existing SEO title of page B as-is.'),
    newDescription: z.string().describe('For DIFFERENTIATE: new meta description. For other actions: the current/existing meta description of page B as-is.'),
    newFocusKeyword: z.string().describe('For DIFFERENTIATE: new unique focus keyword. For other actions: the current focus keyword or main keyword of page B.'),
    targetAngle: z.string().describe('The detected search intent / angle this page currently serves (e.g. "informational guide about X", "product comparison for Y")'),
  }),
...
IMPORTANT: The current year is ${new Date().getFullYear()}. If you include a year in titles, descriptions, or content, use ${new Date().getFullYear()} unless the content specifically refers to a different year.
...
- For pageAChanges and pageBChanges: fill with the CURRENT/EXISTING titles, descriptions, and focus keywords of each page (do not write "not relevant" or similar filler text). Also fill targetAngle with the detected search intent each page currently serves.
...
- For pageAChanges and pageBChanges: fill with the CURRENT/EXISTING titles, descriptions, and focus keywords (do not write "not relevant" or filler text). Also fill targetAngle with the detected search intent each page currently serves.
- The secondary page will be redirected (301) or marked with canonical tag

IMPORTANT for mergeInstructions: Write merge instructions using clear, natural language that a content editor can understand. 
Do NOT reference internal field names like "mergedPageChanges", "pageAChanges", "pageBChanges", "newTitle", "newDescription", "primaryPostId", or similar technical/code terms.
Instead, write instructions like: "Update the title to...", "Change the SEO description to...", "Combine the introduction sections from both articles", "Keep the pricing table from the first article", etc.
...
- MERGE: Content should be combined into a single comprehensive page`;
  ...
  if (result.recommendedAction === 'MERGE' && !result.mergedPageChanges) {
    result.mergedPageChanges = {
      newTitle: pageA.currentSeo?.title || pageA.title || '',
      newDescription: pageA.currentSeo?.description || '',
      newFocusKeyword: pageA.focusKeyword || pageB.focusKeyword || '',
      targetAngle: result.pageAChanges?.targetAngle || '',
      articleType: 'seo',
...
export async function generateMergedContent(insight, site, proposal, options = {}) {
  try {
    ...
    const secondaryPostId = primaryPostId === proposal.postIdA ? proposal.postIdB : proposal.postIdA;

    const [primaryPost, secondaryPost] = await Promise.all([
      getPost(site, primaryPostId).catch(() => null),
      getPost(site, secondaryPostId).catch(() => null),
    ]);

    const primaryContent = stripHtml(primaryPost?.content?.rendered || '').slice(0, 8000);
    const secondaryContent = stripHtml(secondaryPost?.content?.rendered || '').slice(0, 8000);
    const primaryTitle = primaryPost?.title?.rendered || proposal.titleA || '';
    const secondaryTitle = secondaryPost?.title?.rendered || proposal.titleB || '';

    const mergedPageChanges = proposal.recommendation?.mergedPageChanges || {};

    // Generate merged content
    const currentYear = new Date().getFullYear();
    const mergeSystemPrompt = `You are an expert content writer and SEO specialist. You're merging two similar articles into one comprehensive piece.
${langInstruction}
...
Rules:
- Create a single cohesive article that covers all unique information from both sources
- If a year appears in the content (e.g. in titles, headings, or body text), update it to ${currentYear} unless it refers to a specific historical event or date
...
${contentImagesCount > 0 ? `- Provide ${contentImagesCount} contentImageDescriptions: detailed 15-30 word descriptions for each content image, describing specific visual scenes relevant to the surrounding section. Be specific about what should appear in the image.` : ''}`;
    ...
    const mergeUserPrompt = `Merge these two articles into one comprehensive piece:
...
ARTICLE A (${proposal.titleA}):
${primaryContent}

ARTICLE B (${proposal.titleB}):
${secondaryContent}

${mergeInstructions ? `ADDITIONAL INSTRUCTIONS:\n${mergeInstructions}` : ''}

Generate a merged ${wordCount}-word article that combines the best of both sources.`;
    ...
    try {
      result = await generateStructuredResponse({
        ...
        metadata: { articleType, wordCount, primaryPostId, secondaryPostId },
      });
...
export async function applyMergedContent(insight, site, proposal, generatedPost, options = {}) {
  ...
  try {
    ...
    const primaryPostId = proposal.recommendation?.primaryPostId || proposal.postIdA;
    const secondaryPostId = primaryPostId === proposal.postIdA ? proposal.postIdB : proposal.postIdA;
    const secondaryUrl = primaryPostId === proposal.postIdA ? proposal.urlB : proposal.urlA;
    const primaryUrl = primaryPostId === proposal.postIdA ? proposal.urlA : proposal.urlB;
    const primaryPostType = primaryPostId === proposal.postIdA ? (proposal.postTypeA || 'post') : (proposal.postTypeB || 'post');
    const secondaryPostType = primaryPostId === proposal.postIdA ? (proposal.postTypeB || 'post') : (proposal.postTypeA || 'post');
    ...
    actions.push({ type: 'post_updated', status: 'success', detail: primaryUrl });
    ...
    actions.push({ type: 'seo_updated', status: 'success', detail: generatedPost.seoTitle });
    ...
    if (generateFeaturedImages) {
      try {
        ...
        if (imageContext) {
          ...
          if (imageUrl) {
            actions.push({ type: 'featured_image', status: 'success', detail: imageUrl });
          }
        }
      } catch (imgErr) {
    ...
    try {
      ...
      console.log(`[applyMergedContent] Created WP redirect: ${fromPath} -> ${toPath}`);
      actions.push({ type: 'redirect_wp', status: 'success', detail: `${fromPath} → ${toPath}` });
    } catch (redirectErr) {
    ...
    // Save redirect to platform DB
    if (fromPath && toPath) {
      try {
        await prisma.redirection.upsert({
          where: { siteId_sourceUrl: { siteId: site.id, sourceUrl: fromPath } },
          create: { siteId: site.id, sourceUrl: fromPath, targetUrl: toPath, type: 'PERMANENT', isActive: true },
          update: { targetUrl: toPath, type: 'PERMANENT', isActive: true },
        });
        actions.push({ type: 'redirect_platform', status: 'success', detail: `${fromPath} → ${toPath}` });
      } catch (dbErr) {
    ...
    try {
      ...
      actions.push({ type: 'post_trashed', status: 'success', detail: secondaryUrl });
    } catch (trashErr) {
    ...
    // 6. Request GSC re-indexing if connected
    if (googleIntegration?.gscConnected && googleIntegration?.gscSiteUrl) {
      try {
        ...
        if (accessToken) {
          ...
          actions.push({ type: 'gsc_reindex', status: indexResult.success ? 'success' : 'failed', detail: indexResult.detail });
        } else {
