...
export async function applyMergedContent(insight, site, proposal, generatedPost, options = {}) {
  ...
  try {
    ...
    try {
      ...
      actions.push({ type: 'redirect_wp', status: 'success', detail: `${fromPathDisplay} → ${toPathDisplay}` });
    } catch (redirectErr) {
    ...
    if (fromPath && toPath) {
      try {
        ...
        actions.push({ type: 'redirect_platform', status: 'success', detail: `${fromPathDisplay} → ${toPathDisplay}` });
      } catch (dbErr) {
