...
export async function regenerateItem(insight, site, itemIndex) {
  ...
  switch (type) {
    ...
    // The single-page AI previews return { proposals: [...] }; regenerate
    // callers (fix route + modal) expect { proposal } - unwrap the first item.
    case 'aiPageMissingSchema':
      return unwrapSingleProposal(await previewAiPageMissingSchema(insight, site));
    case 'aiAnswerableButNotConcise':
      return unwrapSingleProposal(await previewAiAnswerableButNotConcise(insight, site));
    case 'metaTitleTooShort':
    case 'metaTitleTooLong':
    case 'metaDescTooShort':
    case 'metaDescTooLong':
    ...
    case 'aiEngineGap':
      return unwrapSingleProposal(await previewAiEngineGap(insight, site));
    default:
...
/**
 * Convert a { success, proposals: [...] } preview result into the
 * { success, proposal } shape that regenerate consumers expect.
 */
function unwrapSingleProposal(result) {
  const proposal = result?.proposals?.[0] || null;
  return {
    success: !!result?.success && !!proposal && proposal.status !== 'error' && proposal.status !== 'skipped',
    proposal,
    error: result?.error || (proposal?.status === 'error' || proposal?.status === 'skipped' ? proposal.reason : undefined),
  };
}
...
async function applyAiPageMissingSchemaFix(insight, site, proposals, options = {}) {
  ...
  for (const p of proposals) {
    ...
    try {
      ...
      // The snippet is NOT injected into the site (wp_kses_post strips
      // <script>), so report manual_required - not fixed - and let the UI
      // surface the copy step.
  ...
  const generated = results.filter(r => r.status === 'manual_required').length;
  return {
    success: generated > 0,
    results,
    summary: `Generated schema for ${generated}/${proposals.length} page(s). Copy the snippet into your site's <head> via your theme or a header-injection plugin.`,
  };
}
...
async function previewAiEngineGap(insight, site, userId = null) {
  ...
  try {
    ...
    const schemaProposal = schemaPreview?.proposals?.[0]?.proposed || null;
    const answerableProposal = answerablePreview?.proposals?.[0]?.proposed || null;
    const anyReady = !!(schemaProposal || answerableProposal);
    return {
      success: anyReady,
      proposals: [{
        url: fullUrl,
        postId: resolved.wpPostId,
        postType: resolved.postType,
        status: anyReady ? 'ready' : 'error',
        reason: anyReady
          ? undefined
          : (schemaPreview?.proposals?.[0]?.reason || answerablePreview?.proposals?.[0]?.reason
            || schemaPreview?.error || answerablePreview?.error || 'Generation failed'),
...
async function applyAiEngineGapFix(insight, site, proposals, options = {}) {
  ...
  for (const p of proposals) {
    try {
      ...
      // Schema part is generate-only (the snippet must be copied manually);
      // keep the snippet on the sub-result and never count it as an applied
      // site change on its own.
      let schemaSnippet = null;
      if (p.proposed?.schemaProposal) {
        try {
          ...
          schemaSnippet = r.results?.[0]?.snippet || null;
          subResults.push({ part: 'schema', success: r.success, manual: true, snippet: schemaSnippet });
        } catch (e) {
      ...
      // Only the TL;DR part actually mutates the site. Schema alone means a
      // manual copy step remains - report that honestly instead of 'fixed'.
      const tldrApplied = subResults.some(s => s.part === 'tldr' && s.success);
      const schemaGenerated = subResults.some(s => s.part === 'schema' && s.success);
      results.push({
        ...p,
        status: tldrApplied ? 'fixed' : schemaGenerated ? 'manual_required' : 'error',
        ...(schemaGenerated && schemaSnippet ? { snippet: schemaSnippet, appliedVia: tldrApplied ? undefined : 'manual_copy' } : {}),
        subResults,
      });
    } catch (err) {
      results.push({ ...p, status: 'error', reason: err.message });
  ...
  return {
    success: fixed + manual > 0,
    results,
    summary: `Improved AI-citability on ${fixed + manual}/${proposals.length} page(s)${manual > 0 ? ' (schema snippet requires a manual copy step)' : ''}`,
  };
}
...
export async function applyFreeFix(insight, site, itemIndices = null) {
  ...
  switch (cfg.handler) {
    ...
    case 'rescanCompetitors':
      return applyRescanCompetitorsFix(insight, site, itemIndices);
    case 'slugSuffixCleanup':
      return applySlugSuffixCleanupFix(insight, site, itemIndices);
    default:
