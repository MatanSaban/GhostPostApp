...
async function generateAndUploadFeaturedImage({ site, postId, postType = 'post', title, focusKeyword, targetAngle, imageContext, existingBase64 }) {
  ...
  // Check the update response for inline verification (plugin returns featured_image_id)
  if (updateResult?.featured_image_id && updateResult.featured_image_id !== uploadResult.id) {
    throw new Error(`Featured image mismatch: expected ${uploadResult.id}, got ${updateResult.featured_image_id}`);
  }
  ...
  try {
    ...
    const actualThumbId = verifyPost?.featuredImageId || verifyPost?.featured_image_id;
