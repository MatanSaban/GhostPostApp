 ...
 */
async function generateAndUploadFeaturedImage({ site, postId, postType = 'post', title, focusKeyword, targetAngle, imageContext }) {
...
export async function generateMergedContent(insight, site, proposal, options = {}) {
  try {
    ...
    const allPages = proposal.pages || [
      { postId: proposal.postIdA, title: proposal.titleA },
      { postId: proposal.postIdB, title: proposal.titleB },
    ].filter(pg => pg.postId);
    const secondaryPages = allPages.filter(pg => pg.postId !== primaryPostId);

    // Fetch content for primary and all secondary pages
    const primaryPost = await getPost(site, primaryPostId).catch(() => null);
    const primaryContent = primaryPost?.content?.rendered || '';
    const primaryContentText = stripHtml(primaryContent);
    const primaryTitle = primaryPost?.title?.rendered || allPages.find(pg => pg.postId === primaryPostId)?.title || proposal.titleA || '';

    const secondaryPostsData = await Promise.all(
      secondaryPages.map(async (pg) => {
        const post = await getPost(site, pg.postId).catch(() => null);
        return {
          postId: pg.postId,
...
export async function applyMergedContent(insight, site, proposal, generatedPost, options = {}) {
  ...
  try {
    ...
    // 6. Internal Link Healing: update all internal links pointing to the trashed URL
    try {
      await healInternalLinks(site, [secondaryUrl], primaryUrl);
      actions.push({ type: 'link_healing', status: 'success', detail: `Healed internal links: ${secondaryUrl} → ${primaryUrl}` });
    } catch (linkErr) {
